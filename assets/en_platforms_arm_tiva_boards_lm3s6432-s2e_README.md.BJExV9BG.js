import{_ as n,c as t,al as r,o}from"./chunks/framework.NFAqBSgQ.js";const _=JSON.parse('{"title":"","description":"","frontmatter":{},"headers":[],"relativePath":"en/platforms/arm/tiva/boards/lm3s6432-s2e/README.md","filePath":"en/platforms/arm/tiva/boards/lm3s6432-s2e/README.md"}'),s={name:"en/platforms/arm/tiva/boards/lm3s6432-s2e/README.md"};function i(a,e,l,d,I,c){return o(),t("div",null,e[0]||(e[0]=[r(`<p>README ^^^^^^</p><p>README for NuttX port to the Stellaris RDK-S2E Reference Design Kit and the MDL-S2E Ethernet to Serial module.</p><p>Contents ^^^^^^^^</p><p>Stellaris RDK-S2E Reference Design Kit Stellaris MDL-S2E Reference Design Configuration Options Configurations</p><p>Stellaris RDK-S2E Reference Design Kit ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^</p><p>The Stellaris RDK-S2E Reference Design Kit includes the following features:</p><p>o MDL-S2E Ethernet to serial module o LM3S6432 in a 10 x 10 mm BGA package for reduced board size o 10/100 Mbit Ethernet port o Auto MDI/MDIX cross-over correction o Traffic and link indicators Serial ports o UART ports include RTS/CTS for flow control o UART0 has RS232 levels, transceiver runs at up to 230.4 Kbaud o UART1 has CMOS/TTL levels, can run at 1.0 Mbaud</p><p>Features of the LM3S6432 Microcontroller</p><p>o 32-bit RISC performance using ARM® Cortex™-M3 v7M architecture - 50-MHz operation - Hardware-division and single-cycle-multiplication - Integrated Nested Vectored Interrupt Controller (NVIC) - 42 interrupt channels with eight priority levels o 96 KB single-cycle flash o 32 KB single-cycle SRAM o Three general-purpose 32-bit timers o Integrated Ethernet MAC and PHY o Two fully programmable 16C550-type UARTs o Three 10-bit channels (inputs) when used as single-ended inputs o Two independent integrated analog comparators o One I2C module o One PWM generator block - One 16-bit counter - Two comparators - Produces two independent PWM signals - One dead-band generator o 0 to 43 GPIOs, depending on user configuration o On-chip low drop-out (LDO) voltage regulator</p><p>GPIO Usage</p><p>PIN SIGNAL Function</p><hr><p>L3 PA0/U0RX UART0 receive M3 PA1/U0TX UART0 transmit E12 PB0/U0CTS UART0 CTS D12 PB1/U0RTS UART0 RTS L5 PA4/SPIRX SPI receive (pin hardwired to U1RX) M5 PA5/SPITX SPI transmit (pin hardwired to U1TX) H2 PD2/U1RX UART1 receive H1 PD3/U1TX UART1 transmit L4 PA3/U1CTS/SPICLK UART1 CTS or SPI clock M4 PA2/U1RTS/SPISEL UART1 RTS or SPI slave select J11 PF0/LED1 Ethernet LED1 (green) J12 PF1/LED0 Ethernet LED0 (yellow) C11 PB2 Transceiver #INVALID C12 PB3 Transceiver #ENABLE A6 PB4 Transceiver ON B7 PB5 Transceiver #OFF</p><p>Stellaris MDL-S2E Reference Design Configuration Options ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^</p><pre><code>CONFIG_ARCH - Identifies the arch/ subdirectory.  This should
   be set to:
...
</code></pre><p>There are configurations for disabling support for interrupts GPIO ports. GPIOH and GPIOJ must be disabled because they do not exist on the LM3S6432. Additional interrupt support can be disabled if desired to reduce memory footprint - GPIOs C-G are not pinned out on the MDL-S2E board.</p><pre><code>CONFIG_TIVA_GPIOA_IRQS=y
...
</code></pre><p>LM3S6432 specific device driver settings</p><pre><code>CONFIG_UARTn_DISABLE
...
</code></pre><p>Configurations ^^^^^^^^^^^^^^</p><p>Each Stellaris MDL-S2E Reference Design configuration is maintained in a sub-directory and can be selected as follow:</p><pre><code>tools/configure.sh lm3s6432-s2e:&lt;subdir&gt;
...
</code></pre>`,25)]))}const h=n(s,[["render",i]]);export{_ as __pageData,h as default};
