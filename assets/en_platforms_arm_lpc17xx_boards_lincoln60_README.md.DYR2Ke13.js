import{_ as n,c as t,al as o,o as i}from"./chunks/framework.NFAqBSgQ.js";const h=JSON.parse('{"title":"","description":"","frontmatter":{},"headers":[],"relativePath":"en/platforms/arm/lpc17xx/boards/lincoln60/README.md","filePath":"en/platforms/arm/lpc17xx/boards/lincoln60/README.md"}'),s={name:"en/platforms/arm/lpc17xx/boards/lincoln60/README.md"};function r(a,e,_,C,l,d){return i(),t("div",null,e[0]||(e[0]=[o(`<p>README ^^^^^^</p><p>README for NuttX port to the Micromint Lincoln 60 board</p><p>Contents ^^^^^^^^</p><p>Lincoln 60 development board Lincoln 60 Configuration Options USB Host Configuration Configurations</p><p>Lincoln 60 board ^^^^^^^^^^^^^^^^</p><p>Memory Map ----------</p><p>Block Start Length Name Address --------------------- ---------- ------ Internal flash 0x00000000 512K RAM 0x10000000 32K RAM1 0x2007C000 16K RAM2 0x20080000 16K</p><p>GPIO Usage: -----------</p><p>GPIO PIN SIGNAL NAME</p><hr><p>P1[18] 32 LED1 P3[26] 26 LED2 P2[10] 53 BTN1</p><p>microSD PIN SIGNAL NAME</p><hr><p>P0[15] J12 3 SPI SCK P0[17] J12 4 SPI MISO P0[18] J12 5 SPI MOSI P0[16] J18 5 SPI slave select</p><p>Console -------</p><p>The Lincoln 60 has two serial connectors. The serial console defaults to COM1 (UART0).</p><p>Lincoln 60 Configuration Options ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^</p><pre><code>CONFIG_ARCH - Identifies the arch/ subdirectory.  This should
   be set to:
...
</code></pre><p>LPC17xx/LPC40xx USB Host Configuration</p><pre><code>CONFIG_LPC17_40_OHCIRAM_SIZE
...
CONFIG_LPC17_40_USBHOST_IOBUFSIZE
  ...
  reside in AHB SRAM.
</code></pre><p>USB Host Configuration ^^^^^^^^^^^^^^^^^^^^^^</p><p>The Lincoln 60 board supports a USB host interface. The hidkbd example can be used to test this interface.</p><p>The NuttShell (NSH) lincoln60 can also be modified in order to support USB host operations. To make these modifications, do the following:</p><ol><li><p>First configure to build the NSH configuration from the top-level NuttX directory:</p><p>./configure lincoln60/nsh</p></li><li><p>Then edit the top-level .config file to enable USB host. Make the following changes:</p><p>CONFIG_LPC17_40_USBHOST=y CONFIG_USBHOST=y CONFIG_SCHED_WORKQUEUE=y</p></li></ol><p>When this change is made, NSH should be extended to support USB flash devices. When a FLASH device is inserted, you should see a device appear in the /dev (pseudo) directory. The device name should be like /dev/sda, /dev/sdb, etc. The USB mass storage device, is present it can be mounted from the NSH command line like:</p><p>ls /dev mount -t vfat /dev/sda /mnt/flash</p><p>Files on the connect USB flash device should then be accessible under the mountpoint /mnt/flash.</p><p>Configurations ^^^^^^^^^^^^^^</p><p>Each Lincoln 60 configuration is maintained in a sub-directory and can be selected as follow:</p><pre><code>tools/configure.sh lincoln60:&lt;subdir&gt;
</code></pre><p>Where <code>&lt;subdir&gt;</code> is one of the following:</p><p>netnsh: Configures the NuttShell (nsh) located at apps/examples/nsh. This configuration is similar to the nsh configuration except that network support is enabled.</p><pre><code>NOTES:
...
</code></pre><p>nsh: Configures the NuttShell (nsh) located at apps/examples/nsh.</p><pre><code>NOTES:
...
</code></pre><p>thttpd-binfs: This builds the THTTPD web server example using the THTTPD and the apps/examples/thttpd application. This version uses the built-in binary format with the BINFS file system and the Union File System.</p><pre><code>NOTES:
...
</code></pre><p>STATUS: 2015-06-06: The BINFS CGI files are seems to be running, but the output that they generate does not appear in the browser window. I am suspecting that the redirected output is not working correctly with the BINFS applications.</p>`,46)]))}const p=n(s,[["render",r]]);export{h as __pageData,p as default};
