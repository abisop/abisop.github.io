import{_ as n,c as t,al as i,o}from"./chunks/framework.NFAqBSgQ.js";const l=JSON.parse('{"title":"","description":"","frontmatter":{},"headers":[],"relativePath":"en/platforms/arm/lpc17xx/boards/mbed/README.md","filePath":"en/platforms/arm/lpc17xx/boards/mbed/README.md"}'),_={name:"en/platforms/arm/lpc17xx/boards/mbed/README.md"};function s(r,e,a,C,d,p){return o(),t("div",null,e[0]||(e[0]=[i(`<p>README ^^^^^^</p><p>README for NuttX port to the mbed.org LPC1768 board (<a href="http://mbed.org/" target="_blank" rel="noreferrer">http://mbed.org/</a>)</p><p>Contents ^^^^^^^^</p><p>mbed Configuration Options USB Host Configuration Configurations</p><p>mbed Configuration Options ^^^^^^^^^^^^^^^^^^^^^^^^^^</p><pre><code>CONFIG_ARCH - Identifies the arch/ subdirectory.  This should
   be set to:
...
</code></pre><p>device into /dev/userleds</p><p>USB Host Configuration ^^^^^^^^^^^^^^^^^^^^^^</p><p>The mbed board can be easily modified to support a USB host interface (Remember to add 2 resistors of 15K to D+ and D- pins).</p><p>The NuttShell (NSH) mbed can also be modified in order to support USB host operations. To make these modifications, do the following:</p><ol><li><p>First configure to build the NSH configuration from the top-level NuttX directory:</p><p>./configure mbed/nsh</p></li><li><p>Then edit the top-level .config file to enable USB host. Make the following changes using &#39;make menuconfig&#39;:</p><p>System Type -&gt; LPC17xx/LPC40xx Peripheral Support CONFIG_LPC17_40_USBHOST=y : USB host controller driver support</p><p>Device Drivers -&gt; USB Host Driver Support CONFIG_USBHOST=y : USB host support CONFIG_USBHOST_ISOC_DISABLE=y : Not needed CONFIG_USBHOST_MSC=y : Mass storage class support</p><p>Library Routines CONFIG_SCHED_WORKQUEUE=y : Enable worker thread</p></li></ol><p>When this change is made, NSH should be extended to support USB flash devices. When a FLASH device is inserted, you should see a device appear in the /dev (pseudo) directory. The device name should be like /dev/sda, /dev/sdb, etc. The USB mass storage device, is present it can be mounted from the NSH command line like:</p><pre><code> ls /dev
 mount -t vfat /dev/sda /mnt/flash
</code></pre><p>Files on the connect USB flash device should then be accessible under the mountpoint /mnt/flash.</p><p>Configurations ^^^^^^^^^^^^^^</p><p>General -------</p><p>Each mbed configuration is maintained in a sub-directory and can be selected as follow:</p><pre><code>tools/configure.sh mbed:&lt;subdir&gt;
</code></pre><p>Where <code>&lt;subdir&gt;</code> is one of the mbed subdirectories described in the following paragraph.</p><p>NOTES:</p><ol><li>These configurations use the mconf-based configuration tool. To change any of these configurations using that tool, you should:</li></ol><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>&lt;!-- --&gt;</span></span></code></pre></div><pre><code>a. Build and install the kconfig-mconf tool.  See nuttx/README.txt
   see additional README.txt files in the NuttX tools repository.

b. Execute &#39;make menuconfig&#39; in nuttx/ in order to start the
   reconfiguration process.
</code></pre><p>Configuration Sub-directories -----------------------------</p><p>nsh: Configures the NuttShell (nsh) located at examples/nsh. The Configuration enables only the serial NSH interfaces. See notes above for enabling USB host support in this configuration.</p>`,35)]))}const N=n(_,[["render",s]]);export{l as __pageData,N as default};
