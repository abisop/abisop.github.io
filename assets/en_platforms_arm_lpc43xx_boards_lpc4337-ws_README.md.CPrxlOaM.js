...
-wire=&lt;probe&gt; specifies the debug probe.  For LPCLink on Windows 7 use
    ...
    the full version.
</code></pre><p>Thus the correct invocation for the LPC4337 under Windows7 would be:</p><pre><code>target extended-remote | crt_emu_lpc18_43_nxp -2 -pLPC4337 -wire=winusb
...
</code></pre><p>Command Line Flash Programming ------------------------------</p><p>The LPC18xx/LPC43xx debug driver can also be used to program the LPC43xx flash directly from the command line. The script flash.sh that may be found in the boards/arm/lpc43xx/lpc4337-ws/scripts directory can do that with a single command line command.</p><p>Executing from SPIFI --------------------</p><p>By default, the configurations here assume that you are executing directly from SRAM.</p><pre><code>CONFIG_LPC43_BOOT_SRAM=y            : Executing in SRAM
...
</code></pre><h1 id="configurations" tabindex="-1">Configurations <a class="header-anchor" href="#configurations" aria-label="Permalink to &quot;Configurations&quot;">​</a></h1><p>Each LPC4337-ws configuration is maintained in a sub-directory and can be selected as follow:</p><pre><code>tools/configure.sh LPC4337-ws:&lt;subdir&gt;
...
</code></pre><ol start="6"><li><p>By default the LPC4337-ws port is configured to run from the onboard flash bank A at 0x1a000000. In order to achieve this, the resulting NuttX binary will need to have a checksum computed over the vector table and then be converted to a hex file which can then be flashed using a debugger such as the Uws through Keil.</p><p>The checksum can be computed using the checksum binary provided with the LPCXpresso IDE software suite as follows:</p><p>./checksum nuttx.bin -p LPC4337 -v</p><p>This will modify the binary file, appending the checksum to the correct place at the end of the vector table.</p><p>The binary must now be converted to a hex file, which can be achieved using the srec_cat utility, which is part of the SRecord package (srecord.sourceforge.net) as follows:</p><p>srec_cat nuttx.bin -binary -offset 0x1a000000 -o nuttx.hex -intel --line-length=44</p><p>Now the hex file can be loaded using a debugger, and the code will execute from flash.</p></li></ol>`,107)]))}const C=t(a,[["render",r]]);export{u as __pageData,C as default};
