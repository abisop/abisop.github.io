import{_ as t,c as n,al as o,o as i}from"./chunks/framework.NFAqBSgQ.js";const p=JSON.parse('{"title":"README","description":"","frontmatter":{},"headers":[],"relativePath":"en/platforms/arm/sama5/boards/sama5d3x-ek/README.md","filePath":"en/platforms/arm/sama5/boards/sama5d3x-ek/README.md"}'),a={name:"en/platforms/arm/sama5/boards/sama5d3x-ek/README.md"};function r(s,e,l,d,h,u){return i(),n("div",null,e[0]||(e[0]=[o(`<h1 id="readme" tabindex="-1">README <a class="header-anchor" href="#readme" aria-label="Permalink to &quot;README&quot;">​</a></h1><p>This README file describes the port of NuttX to the SAMA5D3x-EK development boards. These boards feature the Atmel SAMA5D3 microprocessors. Four different SAMA5D3x-EK kits are available</p><pre><code>- SAMA5D31-EK with the ATSAMA5D31 (http://www.atmel.com/devices/sama5d31.aspx)
...
</code></pre><p>The each consist of an identical base board with different plug-in modules for each CPU. I also have a 5 inch LCD for my SAMA5D3x-EK, Atmel 5.0_WVGA_R_AEA_DM, Rev. B, but this LCD is not yet generally available as of this writing.</p><p>I have seen SAMA5D3x-EK boards with different LCDs attached. One of the more common LCDS is the 7 inch TM0000 TFT LCD controller with MaXTouch multi-touch, capacitive touchscreen (<a href="http://www.pdaatl.com/doc/tm7000.pdf" target="_blank" rel="noreferrer">http://www.pdaatl.com/doc/tm7000.pdf</a>). That is NOT the LCD supported by this configuration; the LCD used here is smaller and has a resistive touchscreen that interfaces via the SAMA5D3 ADC interface. Refer to the SAMA5D4-EK for TM0000 support that can be back-ported to the SAMA5D3x-EK.</p><pre><code>SAMA5D3 Family
...
</code></pre><p>NOTES: By closing JP9 (BMS == 0), you can force the board to boot directly to NOR FLASH. Executing from other memories will require that you provide a special code header so that you code can be recognized as a boot-able image by the ROM bootloader.</p><h1 id="creating-and-using-norboot" tabindex="-1">Creating and Using NORBOOT <a class="header-anchor" href="#creating-and-using-norboot" aria-label="Permalink to &quot;Creating and Using NORBOOT&quot;">​</a></h1><p>In order to have more control of debugging code that runs out of NOR FLASH, I created the sama5d3x-ek/norboot configuration. That configuration is described below under &quot;Configurations.&quot;</p><p>Here are some general instructions on how to build an use norboot:</p><p>Building: 1. Remove any old configurations (if applicable).</p><pre><code>   cd &lt;nuttx&gt;
   make distclean
</code></pre><ol start="2"><li><p>Install and build the norboot configuration. This steps will establish the norboot configuration and setup the PATH variable in order to do the build:</p><p>tools/configure.sh sama5d3x-ek:<code>&lt;subdir&gt;</code></p><p>Before building, make sure the PATH environment variable includes the correct path to the directory than holds your toolchain binaries.</p><p>NOTE: Be aware that the default norboot also disables the watchdog. Since you will not be able to re-enable the watchdog later, you may need to set CONFIG_SAMA5_WDT=y in the NuttX configuration file.</p><p>Then make norboot:</p><p>make</p><p>This will result in an ELF binary called &#39;nuttx&#39; and also HEX and binary versions called &#39;nuttx.hex&#39; and &#39;nuttx.bin&#39;.</p></li><li><p>Rename the binaries. Since you will need two versions of NuttX: this norboot version that runs in internal SRAM and another under test in NOR FLASH, I rename the resulting binary files so that they can be distinguished:</p><p>mv nuttx norboot mv nuttx.hex norboot.hex mv nuttx.bin norboot.bin</p></li><li><p>Build your NOR configuration and write this into NOR FLASH. Here, for example, is how you would create the NSH NOR configuration:</p><p>cd <code>&lt;nuttx&gt;</code> make distclean # Remove the norboot configuration tools/configure.sh sama5d3x-ek/nsh # Establish the NSH configuration make # Build the NSH configuration</p><p>Then use SAM-BA to write the nuttx.bin binary into NOR FLASH. This will involve holding the CS_BOOT button and power cycling to start the ROM loader. The SAM-BA serial connection will be on the device USB port, not the debug USB port. Follow the SAM-BA instruction to write the nuttx.bin binary to NOR FLASH.</p></li><li><p>Restart the system without holding CS_BOOT to get back to the normal debug setup.</p></li><li><p>Then start the J-Link GDB server and GDB. In GDB, I do the following:</p><p>(gdb) mon reset # Reset and halt the CPU (gdb) load norboot # Load norboot into internal SRAM (gdb) mon go # Start norboot (gdb) mon halt # Break in (gdb) mon reg pc = 0x10000040 # Set the PC to NOR flash entry point (gdb) mon go # And jump into NOR flash</p><p>The norboot program can also be configured to jump directly into NOR FLASH without requiring the final halt and go by setting CONFIG_SAMA5D3XEK_NOR_START=y in the NuttX configuration. However, since I have been debugging the early boot sequence, the above sequence has been most convenient for me since it allows me to step into the program in NOR.</p></li><li><p>An option is to use the SAM-BA tool to write the NORBOOT image into Serial FLASH. Then, the system will boot from Serial FLASH by copying the NORBOOT image in SRAM which will run and then start the image in NOR FLASH automatically. This is a very convenient usage!</p><p>NOTES: (1) There is jumper on the CM module that must be closed to enable use of the AT25 Serial Flash. (2) If using SAM-BA, make sure that you load the NOR boot program into the boot area via the pull- down menu.</p></li></ol><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>&lt;!-- --&gt;</span></span></code></pre></div><pre><code>STATUS:
...
</code></pre><p>STATUS: 2014-3-30: These instructions were adapted from the Linux4SAM website but have not yet been used.</p><h1 id="buttons-and-leds" tabindex="-1">Buttons and LEDs <a class="header-anchor" href="#buttons-and-leds" aria-label="Permalink to &quot;Buttons and LEDs&quot;">​</a></h1><h2 id="buttons" tabindex="-1">Buttons <a class="header-anchor" href="#buttons" aria-label="Permalink to &quot;Buttons&quot;">​</a></h2><p>There are five push button switches on the SAMA5D3X-EK base board:</p><pre><code>1. One Reset, board reset (BP1)
...
</code></pre><p>Only the momentary push button is controllable by software (labeled &quot;PB_USER1&quot; on the board):</p><pre><code>- PE27.  Pressing the switch connects PE27 to grounded.  Therefore, PE27
  must be pulled high internally.  When the button is pressed the SAMA5
  will sense &quot;0&quot; is on PE27.
</code></pre><h2 id="leds" tabindex="-1">LEDs <a class="header-anchor" href="#leds" aria-label="Permalink to &quot;LEDs&quot;">​</a></h2><p>There are two LEDs on the SAMA5D3 series-CM board that can be controlled by software. A blue LED is controlled via PIO pins. A red LED normally provides an indication that power is supplied to the board but can also be controlled via software.</p><pre><code>PE25.  This blue LED is pulled high and is illuminated by pulling PE25
low.
...
</code></pre><p>These LEDs are not used by the board port unless CONFIG_ARCH_LEDS is defined. In that case, the usage by the board port is defined in include/board.h and src/sam_leds.c. The LEDs are used to encode OS-related events as follows when the red LED (PE24) is available:</p><pre><code>SYMBOL                Meaning                     LED state
                                                Blue     Red
-------------------  -----------------------  -------- --------
...
</code></pre><p>If CONFIG_SAMA5D3XEK_NOREDLED=y, then the red LED is not used by the system and the controls are as follows:</p><pre><code>SYMBOL                Meaning                     LED state
                                                Blue        Red
-------------------  -----------------------  ----------- -----------
...
LED_IDLE             MCU is is sleep mode       Not used  &quot; &quot; &quot;  &quot;
</code></pre><p>Thus if the blue LED is statically on, NuttX has successfully booted and is, apparently, running normally. If the red (or blue) LED is flashing at approximately 2Hz, then a fatal error has been detected and the system has halted.</p><h1 id="serial-consoles" tabindex="-1">Serial Consoles <a class="header-anchor" href="#serial-consoles" aria-label="Permalink to &quot;Serial Consoles&quot;">​</a></h1><h2 id="usart1" tabindex="-1">USART1 <a class="header-anchor" href="#usart1" aria-label="Permalink to &quot;USART1&quot;">​</a></h2><p>By default USART1 is used as the NuttX serial console in all configurations (unless otherwise noted). USART1 is buffered with an RS-232 Transceiver (Analog Devices ADM3312EARU) and connected to the DB-9 male socket (J8).</p><pre><code>USART1 Connector J8
-------------------------------
...
</code></pre><h2 id="hardware-uart-via-cdc" tabindex="-1">Hardware UART via CDC <a class="header-anchor" href="#hardware-uart-via-cdc" aria-label="Permalink to &quot;Hardware UART via CDC&quot;">​</a></h2><p>&quot;J-Link-OB-ATSAM3U4C comes with an additional hardware UART that is accessible from a host via CDC which allows terminal communication with the target device. This feature is enabled only if a certain port (CDC disabled, PA25, pin 24 on J-Link-OB-ATSAM3U4C) is NOT connected to ground (open).</p><pre><code>- Jumper JP16 not fitted: CDC is enabled
- Jumper JP16 fitted : CDC is disabled&quot;
</code></pre><h1 id="networking" tabindex="-1">Networking <a class="header-anchor" href="#networking" aria-label="Permalink to &quot;Networking&quot;">​</a></h1><p>Networking support via the can be added to NSH by selecting the following configuration options. The SAMA5D3x supports two different Ethernet MAC peripherals: (1) The 10/100Base-T EMAC peripheral and (2) the 10/100/1000Base-T GMAC peripheral. Only the SAMA5D31 and SAMAD35 support the EMAC peripheral; Only the SAMA5D33, SAMA5D34, and SAMA5D35 support the GMAC perpheral! NOTE that the SAMA5D35 supports both!</p><p>Selecting the EMAC peripheral -----------------------------</p><p>System Type CONFIG_ARCH_CHIP_ATSAMA5D31=y : SAMA5D31 or SAMAD35 support EMAC CONFIG_ARCH_CHIP_ATSAMA5D35=y : (others do not)</p><p>System Type -&gt; SAMA5 Peripheral Support CONFIG_SAMA5_EMACA=y : Enable the EMAC (type A) peripheral</p><p>System Type -&gt; EMAC device driver options CONFIG_SAMA5_EMAC_NRXBUFFERS=16 : Set aside some RS and TX buffers CONFIG_SAMA5_EMAC_NTXBUFFERS=4 CONFIG_SAMA5_EMAC_PHYADDR=1 : KSZ8021/31 PHY is at address 1 CONFIG_SAMA5_EMAC_AUTONEG=y : Use autonegotiation CONFIG_SAMA5_EMAC_RMII=y : Either MII or RMII interface should work CONFIG_SAMA5_EMAC_PHYSR=30 : Address of PHY status register on KSZ8021/31 CONFIG_SAMA5_EMAC_PHYSR_ALTCONFIG=y : Needed for KSZ8021/31 CONFIG_SAMA5_EMAC_PHYSR_ALTMODE=0x7 : &quot; &quot; &quot; &quot; &quot; &quot; CONFIG_SAMA5_EMAC_PHYSR_10HD=0x1 : &quot; &quot; &quot; &quot; &quot; &quot; CONFIG_SAMA5_EMAC_PHYSR_100HD=0x2 : &quot; &quot; &quot; &quot; &quot; &quot; CONFIG_SAMA5_EMAC_PHYSR_10FD=0x5 : &quot; &quot; &quot; &quot; &quot; &quot; CONFIG_SAMA5_EMAC_PHYSR_100FD=0x6 : &quot; &quot; &quot; &quot; &quot; &quot;</p><p>PHY selection. Later in the configuration steps, you will need to select the KSZ8021/31 PHY for EMAC (See below)</p><p>Selecting the GMAC peripheral -----------------------------</p><p>System Type CONFIG_ARCH_CHIP_ATSAMA5D33=y : SAMA5D31, SAMA5D33 and SAMAD35 CONFIG_ARCH_CHIP_ATSAMA5D34=y : support GMAC (others do not) CONFIG_ARCH_CHIP_ATSAMA5D35=y :</p><p>System Type -&gt; SAMA5 Peripheral Support CONFIG_SAMA5_GMAC=y : Enable the GMAC peripheral</p><p>System Type -&gt; GMAC device driver options CONFIG_SAMA5_GMAC_NRXBUFFERS=16 : Set aside some RS and TX buffers CONFIG_SAMA5_GMAC_NTXBUFFERS=4 CONFIG_SAMA5_GMAC_PHYADDR=1 : KSZ8051 PHY is at address 1 CONFIG_SAMA5_GMAC_AUTONEG=y : Use autonegotiation</p><p>If both EMAC and GMAC are selected, you will also need:</p><pre><code>CONFIG_SAMA5_GMAC_ISETH0=y           : GMAC is &quot;eth0&quot;; EMAC is &quot;eth1&quot;
</code></pre><p>PHY selection. Later in the configuration steps, you will need to select the KSZ9021/31 PHY for GMAC (See below)</p><p>Common configuration settings -----------------------------</p><p>Networking Support CONFIG_NET=y : Enable Networking CONFIG_NET_SOCKOPTS=y : Enable socket operations CONFIG_NET_ETH_PKTSIZE=562 : Maximum packet size 1518 is more standard CONFIG_NET_TCP=y : Enable TCP/IP networking CONFIG_NET_TCPBACKLOG=y : Support TCP/IP backlog CONFIG_NET_UDP=y : Enable UDP networking CONFIG_NET_ICMP=y : Enable ICMP networking CONFIG_NET_ICMP_SOCKET=y : Needed for NSH ping command : Defaults should be okay for other options Device drivers -&gt; Network Device/PHY Support CONFIG_NETDEVICES=y : Enabled PHY selection CONFIG_ETH0_PHY_KSZ8051=y : Select the KSZ8051 PHY (for EMAC), OR CONFIG_ETH0_PHY_KSZ90x1=y : Select the KSZ9021/31 PHY (for GMAC)</p><p>Application Configuration -&gt; Network Utilities CONFIG_NETDB_DNSCLIENT=y : Enable host address resolution CONFIG_NETUTILS_TELNETD=y : Enable the Telnet daemon CONFIG_NETUTILS_TFTPC=y : Enable TFTP data file transfers for get and put commands CONFIG_NETUTILS_NETLIB=y : Network library support is needed CONFIG_NETUTILS_WEBCLIENT=y : Needed for wget support : Defaults should be okay for other options Application Configuration -&gt; NSH Library CONFIG_NSH_TELNET=y : Enable NSH session via Telnet CONFIG_NSH_IPADDR=0x0a000002 : Select an IP address CONFIG_NSH_DRIPADDR=0x0a000001 : IP address of gateway/host PC CONFIG_NSH_NETMASK=0xffffff00 : Netmask CONFIG_NSH_NOMAC=y : Need to make up a bogus MAC address : Defaults should be okay for other options</p><p>Using the network with NSH --------------------------</p><p>So what can you do with this networking support? First you see that NSH has several new network related commands:</p><pre><code>ifconfig, ifdown, ifup:  Commands to help manage your network
get and put:             TFTP file transfers
wget:                    HTML file transfers
ping:                    Check for access to peers on the network
Telnet console:          You can access the NSH remotely via telnet.
</code></pre><p>You can also enable other add on features like full FTP or a Web Server or XML RPC and others. There are also other features that you can enable like DHCP client (or server) or network name resolution.</p><p>By default, the IP address of the SAMA5D3x-EK will be 10.0.0.2 and it will assume that your host is the gateway and has the IP address 10.0.0.1.</p><pre><code>nsh&gt; ifconfig
...
</code></pre><p>NOTE: In this configuration is is normal to have packet loss &gt; 0% the first time you ping due to the default handling of the ARP table.</p><p>On the host side, you should also be able to ping the SAMA5D3x-EK:</p><pre><code>$ ping 10.0.0.2
...
</code></pre><h1 id="at25-serial-flash" tabindex="-1">AT25 Serial FLASH <a class="header-anchor" href="#at25-serial-flash" aria-label="Permalink to &quot;AT25 Serial FLASH&quot;">​</a></h1><p>Connections -----------</p><p>Both the Ronetix and Embest versions of the SAMAD3x CPU modules include an Atmel AT25DF321A, 32-megabit, 2.7-volt SPI serial flash. The SPI connection is as follows:</p><pre><code>AT25DF321A      SAMA5
--------------- -----------------------------------------------
SI              PD11 SPI0_MOSI
SO              PD10 SPI0_MIS0
SCK             PD12 SPI0_SPCK
/CS             PD13 via NL17SZ126 if JP1 is closed (See below)
</code></pre><p>JP1 and JP2 seem to related to /CS on the Ronetix board, but the usage is less clear. For the Embest module, JP1 must be closed to connect /CS to PD13; on the Ronetix schematic, JP11 seems only to bypass a resistor (may not be populated?). I think closing JP1 is correct in either case.</p><p>Configuration -------------</p><p>The Embest or Ronetix CPU module includes an Atmel AT25DF321A, 32-megabit, 2.7-volt SPI serial flash. Support for that serial FLASH can be enabled in these configurations. These are the relevant configuration settings:</p><pre><code>System Type -&gt; SAMA5 Peripheral Support
...
</code></pre><p>NOTE that you must close JP1 on the Embest/Ronetix board in order to enable the AT25 FLASH chip select.</p><p>You can then format the AT25 FLASH for a FAT file system and mount the file system at /mnt/at25 using these NSH commands:</p><pre><code>nsh&gt; mkfatfs /dev/mtdblock0
...
This is a test
</code></pre><p>NOTE: It appears that if Linux runs out of NAND, it will destroy the contents of the AT25.</p><h1 id="hsmci-card-slots" tabindex="-1">HSMCI Card Slots <a class="header-anchor" href="#hsmci-card-slots" aria-label="Permalink to &quot;HSMCI Card Slots&quot;">​</a></h1><p>Physical Slots --------------</p><p>The SAMA5D3x-EK provides a two SD memory card slots: (1) a full size SD card slot (J7 labelled MCI0), and (2) a microSD memory card slot (J6 labelled MCI1).</p><p>The full size SD card slot connects via HSMCI0. The card detect discrete is available on PD17 (pulled high). The write protect discrete is tied to ground (via PP6) and not available to software. The slot supports 8-bit wide transfer mode, but the NuttX driver currently uses only the 4-bit wide transfer mode</p><pre><code>PD17 MCI0_CD
PD1  MCI0_DA0
PD2  MCI0_DA1
PD3  MCI0_DA2
PD4  MCI0_DA3
...
PD9  MCI0_CK
PD0  MCI0_CDA
</code></pre><p>The microSD connects vi HSMCI1. The card detect discrete is available on PD18 (pulled high):</p><pre><code>PD18  MCI1_CD
PB20  MCI1_DA0
PB21  MCI1_DA1
PB22  MCI1_DA2
PB23  MCI1_DA3
...
</code></pre><h1 id="auto-mounter" tabindex="-1">Auto-Mounter <a class="header-anchor" href="#auto-mounter" aria-label="Permalink to &quot;Auto-Mounter&quot;">​</a></h1><p>NuttX implements an auto-mounter than can make working with SD cards easier. With the auto-mounter, the file system will be automatically mounted when the SD card is inserted into the HSMCI slot and automatically unmounted when the SD card is removed.</p><p>The auto-mounter is enable with:</p><pre><code>  CONFIG_FS_AUTOMOUNTER=y
</code></pre><p>However, to use the automounter you will to provide some additional board-level support. See boards/arm/sama5/sama5d4-ek for and example of how you might do this.</p><p>WARNING: SD cards should never be removed without first unmounting them. This is to avoid data and possible corruption of the file system. Certainly this is the case if you are writing to the SD card at the time of the removal. If you use the SD card for read-only access, however, then I cannot think of any reason why removing the card without mounting would be harmful.</p><h1 id="usb-ports" tabindex="-1">USB Ports <a class="header-anchor" href="#usb-ports" aria-label="Permalink to &quot;USB Ports&quot;">​</a></h1><p>The SAMA5D3 series-MB features three USB communication ports:</p><pre><code>* Port A Host High Speed (EHCI) and Full Speed (OHCI) multiplexed with
...
* Port C Host Full Speed (OHCI) only standard type A connector, J19
  lower port
</code></pre><p>All three USB host ports are equipped with 500 mA high-side power switch for self-powered and bus powered applications. The USB device port feature VBUS inserts detection function.</p><p>Port A ------</p><pre><code>PIO  Signal Name Function
---- ----------- -------------------------------------------------------
...
Application Configuration -&gt; System NSH Add-Ons:
  ...
  CONFIG_USBMONITOR_PRIORITY=50    : USB monitor daemon priority
  CONFIG_USBMONITOR_INTERVAL=1     : Dump trace data every second
</code></pre><p>NOTE: If USB debug output is also enabled, both outputs will appear on the serial console. However, the debug output will be asynchronous with the trace output and, hence, difficult to interpret.</p><h1 id="nor-flash-support" tabindex="-1">NOR FLASH Support <a class="header-anchor" href="#nor-flash-support" aria-label="Permalink to &quot;NOR FLASH Support&quot;">​</a></h1><p>Most of these configurations execute out of CS0 NOR flash and can only be loaded via SAM-BA. These are the relevant configuration options the define the NOR FLASH configuration:</p><pre><code>CONFIG_SAMA5_BOOT_CS0FLASH=y            : Boot from FLASH on CS0
...
</code></pre><p>NOTE: In order to boot in this configuration, you need to close the BMS jumper.</p><p>STATUS: I have been unable to execute these configurations from NOR FLASH by closing the BMS jumper (J9). As far as I can tell, this jumper does nothing on my board??? So I have been using the norboot configuration exclusively to start the program-under-test in NOR FLASH (see the section entitled &quot;Creating and Using NORBOOT&quot; above.)</p><h1 id="sdram-support" tabindex="-1">SDRAM Support <a class="header-anchor" href="#sdram-support" aria-label="Permalink to &quot;SDRAM Support&quot;">​</a></h1><p>SRAM Heap Configuration -----------------------</p><p>In these configurations, .data and .bss are retained in ISRAM. SDRAM can be initialized and included in the heap. Relevant configuration settings:</p><pre><code>System Type-&gt;ATSAMA5 Peripheral Support
...
</code></pre><h1 id="can-usage" tabindex="-1">CAN Usage <a class="header-anchor" href="#can-usage" aria-label="Permalink to &quot;CAN Usage&quot;">​</a></h1><p>I planned to verify CAN using the IXXAT USB-to-CAN Compact. This section provides miscellaneous CAN-related notes, mostly to myself but perhaps of interest to others.</p><p>[Unfortunately, as of this writing, I still do not have a proper CAN test bed to verify the CAN driver.]</p><p>CAN Configuration -----------------</p><p>The following steps illustrate how to enable CAN0 and/or CAN1 in the NuttX configuration:</p><pre><code>System Type -&gt; SAMA5 Peripheral Support
...
</code></pre><h2 id="receive-mailboxes-and-address-filtering" tabindex="-1">Receive Mailboxes and Address Filtering <a class="header-anchor" href="#receive-mailboxes-and-address-filtering" aria-label="Permalink to &quot;Receive Mailboxes and Address Filtering&quot;">​</a></h2><p>The SAMA5 CAN0 peripheral supports 8 mailboxes that can be used for sending and receiving messages. Note that the number of dedicated receive mailboxes (CONFIG_SAMA5_CANn_NRECVMB) was set to one in the above configuration. This could be set to any value from 1 to 3 (the upper limit of 3 is purely arbrary and can be increased with some minor code enhancement). The remainder can be configured dynamically to send CAN messages.</p><p>Why would you want to use more than one receive mailbox? There are two reasons. Multiple receive mailboxes might needed to either (1) receive bursts of messages, or (2) to support multiple groups of messages filtered on message ID.</p><p>You must also specify the address filtering for each dedicated receive mailbox:</p><pre><code>System Type -&gt; CAN Drive Support
...
</code></pre><p>Only messages that have IDs that match the CONFIG_SAMA5_CANn_ADDRn when both the received and the configured address are masked by CONFIG_SAMA5_CANn_MASKn will be accepted. For example, if the mask is all ones, then only messages with exact address matches will be accepted; if the mask is all zeroes than any address will be accepted.</p><p>CAN connectors --------------</p><p>CAN1 and CAN2 are available via RJ-11 connectors on the SAMA5D3x-EK. Each is wired as follows. Also shown below is the matching pins if you want connect the CAN to a device that uses an DB-9 connector (Such as the IXXAT USB-to-CAN Compact). Both connector types (as well as RJ-45) are common.</p><pre><code>                +----------+     RJ-11       DB-9
                |    O     |     ----------- --------------
</code></pre><p>+------------+ | | Pin 1 3v3 Pin 1 N/C | +--+ | | o5 | Pin 2 5v Pin 2 CANL | | | | | o9 | Pin 3 N/C Pin 3 GND | +-+ +-+ | | o4 | Pin 4 CANL Pin 4 N/C | | | | | o8 | Pin 5 CANH Pin 5 N/C | |654321| | | o3 | Pin 6 N/C Pin 6 N/C | |oooooo| | | o7 | Pin 7 CANH | +------+ | | o2 | Pin 8 N/C +------------+ | o6 | Pin 9 CANV+ (N/C on IXXAT) RJ-11 Female | x1 | | | | O | +----------+ DB-9 Male</p><h1 id="sama5-adc-support" tabindex="-1">SAMA5 ADC Support <a class="header-anchor" href="#sama5-adc-support" aria-label="Permalink to &quot;SAMA5 ADC Support&quot;">​</a></h1><h2 id="basic-driver-configuration" tabindex="-1">Basic driver configuration <a class="header-anchor" href="#basic-driver-configuration" aria-label="Permalink to &quot;Basic driver configuration&quot;">​</a></h2><p>ADC support can be added to the NSH configuration. However, there are no ADC input pins available to the user for ADC testing (the touchscreen ADC inputs are intended for other functionality). Because of this, there is not much motivation to enable ADC support on the SAMA5D3x-EK. This paragraph is included here, however, for people using a custom SAMA5D3x board that requires ADC support.</p><pre><code>System Type -&gt; SAMA5 Peripheral Support
...
</code></pre><p>You can set the RTC using the NSH date command:</p><pre><code>NuttShell (NSH) NuttX-7.3
nsh&gt; help date
...
</code></pre><h1 id="touchscreen-testing" tabindex="-1">Touchscreen Testing <a class="header-anchor" href="#touchscreen-testing" aria-label="Permalink to &quot;Touchscreen Testing&quot;">​</a></h1><p>You can enable the touchscreen by modifying the configuration in the following ways:</p><pre><code>System Type:
...
</code></pre><p>You might want to tinker with the SWAPXY and THRESHX and THRESHY settings to get the result that you want.</p><pre><code>Drivers:
...
</code></pre><h1 id="i2s-audio-support" tabindex="-1">I2S Audio Support <a class="header-anchor" href="#i2s-audio-support" aria-label="Permalink to &quot;I2S Audio Support&quot;">​</a></h1><p>The SAMA5D3x-EK has two devices on-board that can be used for verification of I2S functionality: HDMI and a WM8904 audio CODEC. As of this writing, the I2S driver is present, but there are not drivers for either the HDMI or the WM8904.</p><p>WM8904 Audio CODEC Interface ----------------------------</p><pre><code>------------- ---------------- ----------------- ----------------------
...
</code></pre><p>WM8904 Configuration --------------------</p><pre><code>System Type -&gt; SAMA5 Peripheral Support
...
</code></pre><p>The NxPlayer ------------</p><p>The NxPlayer is a audio library and command line application for playing audio file. The NxPlayer can be found at apps/system/nxplayer. If you would like to add the NxPlayer, here are some recommended configuration settings.</p><p>First of all, the NxPlayer depends on the NuttX audio subsystem. See the &quot;WM8904 Configuration&quot; above for an example of how the audio subsystem is configured to use the WM8904 CODED with PCM decoding.</p><p>Then the NxPlayer can be enabled as follows:</p><p>System Libraries and NSH Add-Ons -&gt; NxPlayer media player / command line -&gt; CONFIG_SYSTEM_NXPLAYER=y : Build the NxPlayer library CONFIG_NXPLAYER_PLAYTHREAD_STACKSIZE=1500 : Size of the audio player stack CONFIG_NXPLAYER_COMMAND_LINE=y : Build command line application CONFIG_NXPLAYER_INCLUDE_HELP=y : Includes a help command CONFIG_NXPLAYER_INCLUDE_DEVICE_SEARCH=n : (Since there is only one audio device) CONFIG_NXPLAYER_INCLUDE_PREFERRED_DEVICE=y : Only one audio device is supported CONFIG_NXPLAYER_FMT_FROM_EXT=y : (Since only PCM is supported) CONFIG_NXPLAYER_FMT_FROM_HEADER=n : (Since only PCM is supported) CONFIG_NXPLAYER_INCLUDE_MEDIADIR=y : Specify a media directory CONFIG_NXPLAYER_DEFAULT_MEDIADIR=&quot;/music&quot; : See nxplayer configuration CONFIG_NXPLAYER_RECURSIVE_MEDIA_SEARCH=y : Search all sub-directories CONFIG_NXPLAYER_INCLUDE_SYSTEM_RESET=y : Add support for reset command</p><p>You must include the full path to the location where NxPlayer can find the media files. That path is given by CONFIG_NXPLAYER_DEFAULT_MEDIADIR. Here I use the example &quot;/mnt/scard&quot;. That is a location where you could, for example, mount an MMC/SD card driver.</p><p>I2S Loopback Test -----------------</p><p>The I2S driver was verified using a special I2C character driver (at nuttx/drivers/audio/i2schar.c) and a test driver at apps/examples/i2schar. The I2S driver was verified in loopback mode with no audio device.</p><p>This section describes the modifications to the NSH configuration that were used to perform the I2S testing:</p><pre><code>System Type -&gt; SAMA5 Peripheral Support
  CONFIG_SAMA5_SSCO=y              : Enable SSC0 driver support
  CONFIG_SAMA5_DMAC0=y             : DMAC0 required by SSC0
...
Library Routines
  CONFIG_SCHED_WORKQUEUE=y          : Driver needs work queue support
</code></pre><h1 id="sama5d3x-ek-configuration-options" tabindex="-1">SAMA5D3x-EK Configuration Options <a class="header-anchor" href="#sama5d3x-ek-configuration-options" aria-label="Permalink to &quot;SAMA5D3x-EK Configuration Options&quot;">​</a></h1><p>CONFIG_ARCH - Identifies the arch/ subdirectory. This should be set to:</p><pre><code>CONFIG_ARCH=&quot;arm&quot;
...
</code></pre><p>and one of:</p><p>CONFIG_ARCH_CHIP_ATSAMA5D31=y CONFIG_ARCH_CHIP_ATSAMA5D33=y CONFIG_ARCH_CHIP_ATSAMA5D34=y CONFIG_ARCH_CHIP_ATSAMA5D35=y</p><p>CONFIG_ARCH_BOARD - Identifies the boards/ subdirectory and hence, the board that supports the particular chip or SoC.</p><pre><code>CONFIG_ARCH_BOARD=&quot;sama5d3x-ek&quot; (for the SAMA5D3x-EK development board)
</code></pre><p>CONFIG_ARCH_BOARD_name - For use in C code</p><pre><code>CONFIG_ARCH_BOARD_SAMA5D3X_EK=y
...
</code></pre><ol start="2"><li><p>Unless stated otherwise, all configurations generate console output on USART1 (J8).</p></li><li><p>All of these configurations use the Code Sourcery for Windows toolchain (unless stated otherwise in the description of the configuration). That toolchain selection can easily be reconfigured using &#39;make menuconfig&#39;. Here are the relevant current settings:</p><p>Build Setup: CONFIG_HOST_WINDOWS=y : Microsoft Windows CONFIG_WINDOWS_CYGWIN=y : Using Cygwin or other POSIX environment</p><p>System Type -&gt; Toolchain: CONFIG_ARM_TOOLCHAIN_GNU_EABI=y : GNU EABI toolchain</p></li><li><p>The SAMA5Dx is running at 396MHz by default in these configurations. This is because the original timing for the PLLs, NOR FLASH, and SDRAM came from the Atmel NoOS sample code which runs at that rate.</p><p>The SAMA5Dx is capable of running at 528MHz, however, and is easily re-configured:</p><p>Board Selection -&gt; CPU Frequency CONFIG_SAMA5D3XEK_396MHZ=n # Disable 396MHz operation CONFIG_SAMA5D3XEK_528MHZ=y # Enable 528MHz operation</p><p>If you switch to 528MHz, you should also check the loop calibration value in your .config file. Of course, it would be best to re-calibrate the timing loop, but these values should get you in the ballpark:</p><p>CONFIG_BOARD_LOOPSPERMSEC=49341 # Calibrated on SAMA5D3-EK at 396MHz # running from ISRAM CONFIG_BOARD_LOOPSPERMSEC=65775 # Calibrated on SAMA4D3-Xplained at # 528MHz running from SDRAM</p><p>Operation at 528MHz has been verified but is not the default in these configurations because most testing was done at 396MHz. NAND has not been verified at these rates.</p></li><li><p>By default, all of these configurations run from ISRAM or NOR FLASH (as indicated below in each description of the configuration). Operation from SDRAM is also an option as described in the paragraph entitled, &quot;Running NuttX from SDRAM.&quot;</p></li></ol><h2 id="configuration-sub-directories" tabindex="-1">Configuration Sub-directories <a class="header-anchor" href="#configuration-sub-directories" aria-label="Permalink to &quot;Configuration Sub-directories&quot;">​</a></h2><p>Summary: Some of the descriptions below are long and wordy. Here is the concise summary of the available SAMA5D3x-EK configurations:</p><pre><code>demo: This is an NSH configuration that supports as much functionality
...
</code></pre><p>There may be issues with some of these configurations. See the details before of the status of individual configurations.</p><p>Now for the gory details:</p><p>demo:</p><pre><code>This configuration directory provide the NuttShell (NSH).  There are
...
</code></pre><p>hello:</p><pre><code>This configuration directory, performs the (almost) simplest of all
...
STATUS:
   See the To-Do list below
</code></pre><p>norboot: This is a little program to help debug of code in NOR flash. It does the following:</p><pre><code>- It enables and configures NOR FLASH, then
...
STATUS:
  ...
  2013-9-12:  The I2C tool, however, seems to work well.  It successfully
    ...
    integrated.
</code></pre><p>nx:</p><pre><code>A simple test using the NuttX graphics system (NX) that has been used to
...
user interaction.
</code></pre><p>nxplayer</p><pre><code>A command line media player using the on-board WM8904 audio CODEC.
...
</code></pre><p>ov2640:</p><pre><code>A test of the SAMA5 ISI using an OV2640 camera.
</code></pre><h1 id="to-do-list" tabindex="-1">To-Do List <a class="header-anchor" href="#to-do-list" aria-label="Permalink to &quot;To-Do List&quot;">​</a></h1><ol><li><p>Most of these configurations execute from NOR FLASH. I have been unable to execute these configurations from NOR FLASH by closing the BMS jumper (J9). As far as I can tell, this jumper does nothing on my board??? I have been using the norboot configuration to start the program in NOR FLASH (see just above). See &quot;Creating and Using NORBOOT&quot; above.</p><p>UPDATE: It has been confirmed at that there is an issue with the BMS jumper on my board. However, other NuttX users have confirmed operation booting directly into NOR FLASH. So although I cannot confirm this behavior, this appears to be no longer an issue.</p></li><li><p>Neither USB OHCI nor EHCI support Isochronous endpoints. Interrupt endpoint support in the EHCI driver is untested (but works in similar EHCI drivers).</p></li><li><p>HSCMI. CONFIG_MMCSD_MULTIBLOCK_LIMIT=1 is set to disable multi-block transfers because of some issues that I saw during testing. The is very low priority to me but might be important to you if you are need very high performance SD card accesses.</p><p>HSMCI TX DMA is currently disabled for the SAMA5D3. There is some issue with the TX DMA setup (HSMCI TX DMA the same driver works with the SAMA5D4 which has a different DMA subsystem). This is a bug that needs to be resolved.</p><p>UPDATE: This problem may be fixed with a bug correct on 2015-03-15). Need to retest. That change is necessary, but may not be sufficient to solve the problem.</p></li><li><p>I believe that there is an issue when the internal AT25 FLASH is formatted by NuttX. That format works fine with Linux, but does not appear to work with Windows. Reformatting on Windows can resolve this. NOTE: This is not a SAMA5Dx issue.</p><p>UPDATE: Two important bugs were recently fixed in the NuttX FAT formatting function (mkfatfs). It is likely that these fixes will eliminate this issue, but that has not yet been verified.</p></li><li><p>CAN testing has not yet been performed due to issues with cabling. I just do not have a good test bed (or sufficient CAN knowledge) for good CAN testing.</p></li><li><p>There are lots of LCDC hardware features that are not tested with NuttX. The simple NuttX graphics system does not have support for all of the layers and other features of the LCDC.</p></li><li><p>I have a Camera, but there is still no ISI driver. I am not sure what to do with the camera. NuttX needs something like V4L to provide the definition for what a camera driver is supposed to do.</p><p>I will probably develop a test harness for ISI, but it is of only minimal value with no OS infrastructure to deal with images and video.</p></li><li><p>GMAC has only been tested on a 10/100Base-T network. I don&#39;t have a 1000Base-T network to support additional testing.</p></li><li><p>Some drivers may require some adjustments if you intend to run from SDRAM. That is because in this case macros like BOARD_MCK_FREQUENCY are not constants but are instead function calls: The MCK clock frequency is not known in advance but instead has to be calculated from the bootloader PLL configuration.</p><p>As of this writing, all drivers have been converted to run from SDRAM except for the PWM and the Timer/Counter drivers. These drivers use the BOARD_MCK_FREQUENCY definition in more complex ways and will require some minor redesign and re-testing before they can be available.</p></li><li><p>2014-9-20: Failed to build the NxWM configuration:</p></li></ol><p>CC: chip/sam_adc.c chip/sam_adc.c: In function &#39;sam_adc_interrupt&#39;: chip/sam_adc.c:886:21: warning: unused variable &#39;priv&#39; [-Wunused-variable] struct sam_adc_s *priv = &amp;g_adcpriv; ^ chip/sam_adc.c: In function &#39;sam_adc_initialize&#39;: chip/sam_adc.c:1977:7: error: &#39;g_adcdev&#39; undeclared (first use in this function) g_adcdev.ad_ops = &amp;g_adcops; ^ chip/sam_adc.c:1977:7: note: each undeclared identifier is reported only once for each function it appears in chip/sam_adc.c:1977:27: error: &#39;g_adcops&#39; undeclared (first use in this function) g_adcdev.ad_ops = &amp;g_adcops; ^ chip/sam_adc.c:1983:11: error: &#39;struct sam_adc_s&#39; has no member named &#39;dev&#39; priv-&gt;dev = &amp;g_adcdev; ^ chip/sam_adc.c:2090:1: warning: control reaches end of non-void function [-Wreturn-type] } ^</p><p>The failure occurs because there are no ADC channels configured (as there should not be) so SAMA5_ADC_HAVE_CHANNELS is not defined (as it should not be). However, if there are no configured ADC channel, then sam_adc_initialize() does not compile correctly -- and it should not given nature the logic that is in place there now.</p><p>A quick glance at the history of these files does not reveal what the obvious solution is so I will need to come back and revisit this in the future.</p>`,479)]))}const S=t(a,[["render",r]]);export{p as __pageData,S as default};
