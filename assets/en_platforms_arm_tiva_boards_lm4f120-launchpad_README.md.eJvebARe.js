...
</code></pre><p>LEDs ^^^^ The LM4F120 has a single RGB LED. If CONFIG_ARCH_LEDS is defined, then support for the LaunchPad LEDs will be included in the build. See:</p><ul><li><p>boards/arm/tiva/lm4f120-launchpad/include/board.h - Defines LED constants, types and prototypes the LED interface functions.</p></li><li><p>boards/arm/tiva/lm4f120-launchpad/src/lm4f120-launchpad.h - GPIO settings for the LEDs.</p></li><li><p>boards/arm/tiva/lm4f120-launchpad/src/up_leds.c - LED control logic.</p></li></ul><p>OFF: - OFF means that the OS is still initializing. Initialization is very fast so if you see this at all, it probably means that the system is hanging up somewhere in the initialization phases.</p><p>GREEN or GREEN-ish - This means that the OS completed initialization.</p><p>Bluish: - Whenever and interrupt or signal handler is entered, the BLUE LED is illuminated and extinguished when the interrupt or signal handler exits. This will add a BLUE-ish tinge to the LED.</p><p>Redish: - If a recovered assertion occurs, the RED component will be illuminated briefly while the assertion is handled. You will probably never see this.</p><p>Flashing RED: - In the event of a fatal crash, the BLUE and GREEN components will be extinguished and the RED component will FLASH at a 2Hz rate.</p><p>Serial Console ^^^^^^^^^^^^^^</p><p>By default, all configurations use UART0 which connects to the USB VCOM on the DEBUG port on the LM4F120 LaunchPad:</p><pre><code>UART0 RX  - PA.0
UART0 TX  - PA.1
...
</code></pre><p>LM4F120 LaunchPad Configuration Options ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^</p><pre><code>CONFIG_ARCH - Identifies the arch/ subdirectory.  This should
   be set to:
...
</code></pre><p>There are configurations for disabling support for interrupts GPIO ports. GPIOJ must be disabled because it does not exist on the LM4F120. Additional interrupt support can be disabled if desired to reduce memory footprint.</p><pre><code>CONFIG_TIVA_GPIOA_IRQS=y
...
</code></pre><p>LM4F120 specific device driver settings</p><pre><code>CONFIG_UARTn_SERIAL_CONSOLE - selects the UARTn for the
...
</code></pre><p>Configurations ^^^^^^^^^^^^^^</p><p>Each LM4F120 LaunchPad configuration is maintained in a sub-directory and can be selected as follow:</p><pre><code>tools/configure.sh lm4f120-launchpad:&lt;subdir&gt;
...
</code></pre>`,64)]))}const C=n(i,[["render",r]]);export{h as __pageData,C as default};
