...
</code></pre><h1 id="serial-console" tabindex="-1">Serial Console <a class="header-anchor" href="#serial-console" aria-label="Permalink to &quot;Serial Console&quot;">​</a></h1><p>As with most NuttX configurations, the Freedom KL26Z configurations depend on having a serial console to interact with the software. The Freedom KL26Z, however, has no on-board RS-232 drivers so will be necessary to connect the Freedom KL26Z UART pins to an external RS-232 driver board or TTL-to-Serial USB adaptor.</p><p>By default UART0 is used as the serial console on this boards. The UART0 is configured to work with the OpenSDA USB CDC/ACM port:</p><pre><code>------ ------------------------------- -----------------------------
...
</code></pre><p>Ground is available on J2 pin 14. 3.3V is available on J3 and J4.</p><h1 id="freedom-kl26z-specific-configuration-options" tabindex="-1">Freedom KL26Z-specific Configuration Options <a class="header-anchor" href="#freedom-kl26z-specific-configuration-options" aria-label="Permalink to &quot;Freedom KL26Z-specific Configuration Options&quot;">​</a></h1><pre><code>CONFIG_ARCH - Identifies the arch/ subdirectory.  This should
   be set to:
...
</code></pre><p>Individual subsystems can be enabled as follows. These settings are for all of the K25Z100/120 line and may not be available for the MKL26Z128 in particular:</p><p>AHB ---</p><pre><code>CONFIG_KL_PDMA    Peripheral DMA
...
</code></pre><p>K26Z1XX specific device driver settings</p><pre><code>CONFIG_UARTn_SERIAL_CONSOLE - Selects the UARTn (n=0,1,2) for the
  console and ttys0.
...
</code></pre><h1 id="configurations" tabindex="-1">Configurations <a class="header-anchor" href="#configurations" aria-label="Permalink to &quot;Configurations&quot;">​</a></h1><p>Each FREEDOM-KL26Z configuration is maintained in a sub-directory and can be selected as follow:</p><pre><code>tools/configure.sh freedom-kl26z:&lt;subdir&gt;
</code></pre><p>If this is a Windows native build, then configure.bat should be used instead of configure.sh:</p><pre><code>configure.bat freedom-kl26z\\&lt;subdir&gt;
...
</code></pre>`,46)]))}const f=n(a,[["render",r]]);export{p as __pageData,f as default};
