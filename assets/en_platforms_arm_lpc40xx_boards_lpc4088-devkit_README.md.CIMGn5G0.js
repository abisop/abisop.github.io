import{_ as t,c as n,al as o,o as a}from"./chunks/framework.NFAqBSgQ.js";const p=JSON.parse('{"title":"README.txt","description":"","frontmatter":{},"headers":[],"relativePath":"en/platforms/arm/lpc40xx/boards/lpc4088-devkit/README.md","filePath":"en/platforms/arm/lpc40xx/boards/lpc4088-devkit/README.md"}'),i={name:"en/platforms/arm/lpc40xx/boards/lpc4088-devkit/README.md"};function s(r,e,l,h,d,c){return a(),n("div",null,e[0]||(e[0]=[o(`<h1 id="readme-txt" tabindex="-1">README.txt <a class="header-anchor" href="#readme-txt" aria-label="Permalink to &quot;README.txt&quot;">​</a></h1><p>This README file discusses the port of NuttX to the Embedded Artists LPC4088 Developer&#39;s Kit board: See <a href="https://www.embeddedartists.com/products/lpc4088-developers-kit/" target="_blank" rel="noreferrer">https://www.embeddedartists.com/products/lpc4088-developers-kit/</a> This board features the NXP LPC4088FET208 MCU on a carrier board called the LPC4088 OEM Board.</p><h1 id="contents" tabindex="-1">CONTENTS <a class="header-anchor" href="#contents" aria-label="Permalink to &quot;CONTENTS&quot;">​</a></h1><p>o LEDs o Buttons o Serial Console o Using OpenOCD with the Olimex ARM-USB-OCD o Loading Code with the ISP Board o Configuration</p><h1 id="leds" tabindex="-1">LEDs <a class="header-anchor" href="#leds" aria-label="Permalink to &quot;LEDs&quot;">​</a></h1><p>The LPC4088 OEM board has two user LEDs on GPIO pins:</p><pre><code>LED1 : Connected to P2[26]
LED2 : Connected to P2[27]
</code></pre><p>If CONFIG_ARCH_LEDS is not defined, then the user can control the LEDs in any way using the definitions provided in the board.h header file.</p><p>If CONFIG_ARCH_LEDs is defined, then NuttX will control the 2 LEDs on the LPC4088 OEM board. The following definitions describe how NuttX controls the LEDs: LED1 LED2 LED3 LED4 LED_STARTED OFF OFF OFF OFF LED_HEAPALLOCATE ON OFF OFF OFF LED_IRQSENABLED OFF ON OFF OFF LED_STACKCREATED ON ON OFF OFF LED_INIRQ LED3 glows, on while in interrupt LED_SIGNAL LED3 glows, on while in signal handler LED_ASSERTION LED3 glows, on while in assertion LED_PANIC LED3 Flashes at 2Hz LED_IDLE LED glows: ON while active; OFF while sleeping</p><pre><code>Several additional LEDs are available on a PCA9532 port expander, which is
not currently enabled.
</code></pre><h1 id="buttons" tabindex="-1">Buttons <a class="header-anchor" href="#buttons" aria-label="Permalink to &quot;Buttons&quot;">​</a></h1><p>The LPC4088 Developer&#39;s Kit supports a button:</p><pre><code>USER1           : Connected to P2[10]
</code></pre><p>And a Joystick</p><pre><code>JOY_A           : Connected to P2[23]
JOY_B           : Connected to P2[25]
JOY_C           : Connected to P2[26]
JOY_D           : Connected to P2[27]
JOY_CTR         : Connected to P2[22]
</code></pre><p>These can be accessed using the definitions and interfaces defined in the board.h header file.</p><p>Several additional buttons are available on a PCA9532 port expander, which is not currently enabled.</p><h1 id="serial-console" tabindex="-1">Serial Console <a class="header-anchor" href="#serial-console" aria-label="Permalink to &quot;Serial Console&quot;">​</a></h1><p>By Default, UART0 is used as the serial console in all configurations. This may be connected to your computer via the onboard FT232 USB to UART chip.</p><p>As an option, UART1 can also be used for the serial console. You might want, to do this, for example, if you use UART0 for the ISP function and you want to use a different UART for console output. UART1 can be configured as the serial console by changing the configuration as follows:</p><pre><code>System Type:
...
</code></pre><p>In this configuration using UART1, the jumpers JP12 and JP13 must be set to short pins 1 and 2. UART 1 will then be available on the DB9 connector J17.</p><h1 id="using-openocd-with-the-olimex-arm-usb-ocd" tabindex="-1">Using OpenOCD with the Olimex ARM-USB-OCD <a class="header-anchor" href="#using-openocd-with-the-olimex-arm-usb-ocd" aria-label="Permalink to &quot;Using OpenOCD with the Olimex ARM-USB-OCD&quot;">​</a></h1><p>Building OpenOCD under Cygwin:</p><pre><code>Refer to boards/olimex-lpc1766stk/README.txt
...
3. I find that there are often undetected write failures when using
  ...
  not be necessary.
</code></pre><h1 id="loading-code-with-the-isp-board" tabindex="-1">Loading Code with the ISP Board <a class="header-anchor" href="#loading-code-with-the-isp-board" aria-label="Permalink to &quot;Loading Code with the ISP Board&quot;">​</a></h1><p>Users can also load code onto the board using the UART0 USB VCOM chip. I use the FlashMagic program for Windows available here: <a href="http://www.flashmagictool.com/" target="_blank" rel="noreferrer">http://www.flashmagictool.com/</a> . It is so easy to use that no further explanation should be necessary: Just select the LPC4088, the ISP COM port, and the NuttX .hex file and program it.</p><h1 id="configuration" tabindex="-1">CONFIGURATION <a class="header-anchor" href="#configuration" aria-label="Permalink to &quot;CONFIGURATION&quot;">​</a></h1><h2 id="information-common-to-all-configurations" tabindex="-1">Information Common to All Configurations <a class="header-anchor" href="#information-common-to-all-configurations" aria-label="Permalink to &quot;Information Common to All Configurations&quot;">​</a></h2><ol><li><p>These configurations use the mconf-based configuration tool. To change this configuration using that tool, you should:</p><p>a. Build and install the kconfig-mconf tool. See nuttx/README.txt see additional README.txt files in the NuttX tools repository README.txt.</p><p>b. Execute &#39;make menuconfig&#39; in nuttx/ in order to start the reconfiguration process.</p></li><li><p>Most (but not all) configurations use the &quot;GNU Tools for ARM Embedded Processors&quot; that is maintained by ARM:</p><p><a href="https://developer.arm.com/open-source/gnu-toolchain/gnu-rm" target="_blank" rel="noreferrer">https://developer.arm.com/open-source/gnu-toolchain/gnu-rm</a></p><p>unless otherwise stated.</p><p>That toolchain selection can easily be reconfigured using &#39;make menuconfig&#39;. Here are the relevant current settings:</p><p>Build Setup: CONFIG_HOST_WINDOWS=y : Window environment CONFIG_WINDOWS_CYGWIN=y : Cywin under Windows</p><p>System Type -&gt; Toolchain: CONFIG_ARM_TOOLCHAIN_GNU_EABI=y : GNU ARM EABI toolchain</p></li><li><p>By Default, UART0 is used as the serial console in all configurations. This may be connected to your computer via an external RS-232 driver or via the on board USB VCOM chip. See the section above entitled &quot;Serial Console&quot; for other options.</p></li><li><p>An LCD is available for this board, but I don&#39;t have one to test with. If you wish to use any of the configurations below which utilize the LCD, you will need to tweak the LCD pin definitions in board.h.</p></li></ol><h2 id="configuration-directories" tabindex="-1">Configuration Directories <a class="header-anchor" href="#configuration-directories" aria-label="Permalink to &quot;Configuration Directories&quot;">​</a></h2><h2 id="knsh" tabindex="-1">knsh <a class="header-anchor" href="#knsh" aria-label="Permalink to &quot;knsh&quot;">​</a></h2><p>This is identical to the nsh configuration below except that NuttX is built as a kernel-mode, monolithic module and the user applications are built separately. Is is recommended to use a special make command; not just &#39;make&#39; but make with the following two arguments:</p><pre><code>    make pass1 pass2
...
</code></pre><h2 id="nsh" tabindex="-1">nsh <a class="header-anchor" href="#nsh" aria-label="Permalink to &quot;nsh&quot;">​</a></h2><p>Configures the NuttShell (nsh) located at examples/nsh. The Configuration enables both the serial NSH interface as well as the telnet interface over ethernet, with an IP address assigned via DHCP.</p>`,44)]))}const f=t(i,[["render",s]]);export{p as __pageData,f as default};
