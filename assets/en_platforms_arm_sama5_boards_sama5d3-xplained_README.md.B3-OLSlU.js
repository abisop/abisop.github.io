import{_ as t,c as n,al as o,o as i}from"./chunks/framework.NFAqBSgQ.js";const c=JSON.parse('{"title":"README","description":"","frontmatter":{},"headers":[],"relativePath":"en/platforms/arm/sama5/boards/sama5d3-xplained/README.md","filePath":"en/platforms/arm/sama5/boards/sama5d3-xplained/README.md"}'),a={name:"en/platforms/arm/sama5/boards/sama5d3-xplained/README.md"};function r(s,e,l,d,h,u){return i(),n("div",null,e[0]||(e[0]=[o(`<h1 id="readme" tabindex="-1">README <a class="header-anchor" href="#readme" aria-label="Permalink to &quot;README&quot;">​</a></h1><p>This README file describes the port of NuttX to the SAMA5D3-Xplained development board. This board features the Atmel SAMA5D36 microprocessor. See <a href="http://www.atmel.com/devices/sama5d36.aspx" target="_blank" rel="noreferrer">http://www.atmel.com/devices/sama5d36.aspx</a> for further information.</p><pre><code>PARAMETER                 SAMA5D36
------------------------- -------------
Pin Count                 324
...
</code></pre><p>These LEDs are not used by the board port unless CONFIG_ARCH_LEDS is defined. In that case, the usage by the board port is defined in include/board.h and src/sam_leds.c. The LEDs are used to encode OS-related events as follows:</p><pre><code>SYMBOL                Meaning                     LED state
                                                Blue     Red
-------------------  -----------------------  -------- --------
...
</code></pre><p>Thus if the blue LED is statically on, NuttX has successfully booted and is, apparently, running normally. If the red LED is flashing at approximately 2Hz, then a fatal error has been detected and the system has halted.</p><h1 id="serial-console" tabindex="-1">Serial Console <a class="header-anchor" href="#serial-console" aria-label="Permalink to &quot;Serial Console&quot;">​</a></h1><p>UARTS/USARTS ------------</p><pre><code>CONN  LABEL   PIO   UART/USART  FUNCTION
----- ------- ----- ----------- ---------------
...
</code></pre><p>DBGU Interface --------------</p><p>The SAMA5D3 Xplained board has a dedicated serial port for debugging, which is accessible through the 6-pin male header J23.</p><p>PIN PIO Usage</p><hr><p>1 PE13 (available) 2 PB31 DBGU DTXD 3 PB30 DBGU DRXD 4 N/C (may be used by debug interface tool) 5 PE14 (available) 6 GND</p><p>By default the DBUG port is used as the NuttX serial console in all configurations (unless otherwise noted). The DBGU is available at logic levels at pins RXD and TXD of the DEBUG connector (J23). GND is available at J23 and +3.3V is available from J14</p><h1 id="networking" tabindex="-1">Networking <a class="header-anchor" href="#networking" aria-label="Permalink to &quot;Networking&quot;">​</a></h1><p>Networking support via the can be added to NSH by selecting the following configuration options. The SAMA5D36 supports two different Ethernet MAC peripherals: (1) The 10/100Base-T EMAC peripheral and (2) the 10/100/1000Base-T GMAC peripheral. Ethernet over USB using the CDC ECM driver is also supported, and should work on Linux, macOS, and Windows.</p><p>Selecting the EMAC peripheral -----------------------------</p><p>System Type -&gt; SAMA5 Peripheral Support CONFIG_SAMA5_EMACA=y : Enable the EMAC A peripheral</p><p>System Type -&gt; EMAC device driver options CONFIG_SAMA5_EMAC_NRXBUFFERS=16 : Set aside some RS and TX buffers CONFIG_SAMA5_EMAC_NTXBUFFERS=4 CONFIG_SAMA5_EMAC_PHYADDR=1 : KSZ9031 PHY is at address 1 CONFIG_SAMA5_EMAC_AUTONEG=y : Use autonegotiation CONFIG_SAMA5_EMAC_RMII=y : Either MII or RMII interface should work CONFIG_SAMA5_EMAC_PHYSR=30 : Address of PHY status register on KSZ9031 CONFIG_SAMA5_EMAC_PHYSR_ALTCONFIG=y : Needed for KSZ9031 CONFIG_SAMA5_EMAC_PHYSR_ALTMODE=0x7 : &quot; &quot; &quot; &quot; &quot; &quot; CONFIG_SAMA5_EMAC_PHYSR_10HD=0x1 : &quot; &quot; &quot; &quot; &quot; &quot; CONFIG_SAMA5_EMAC_PHYSR_100HD=0x2 : &quot; &quot; &quot; &quot; &quot; &quot; CONFIG_SAMA5_EMAC_PHYSR_10FD=0x5 : &quot; &quot; &quot; &quot; &quot; &quot; CONFIG_SAMA5_EMAC_PHYSR_100FD=0x6 : &quot; &quot; &quot; &quot; &quot; &quot;</p><p>PHY selection. Later in the configuration steps, you will need to select the KSZ9031 PHY for EMAC (See below)</p><p>Selecting the GMAC peripheral -----------------------------</p><p>System Type -&gt; SAMA5 Peripheral Support CONFIG_SAMA5_GMAC=y : Enable the GMAC peripheral</p><p>System Type -&gt; GMAC device driver options CONFIG_SAMA5_GMAC_NRXBUFFERS=16 : Set aside some RS and TX buffers CONFIG_SAMA5_GMAC_NTXBUFFERS=4 CONFIG_SAMA5_GMAC_PHYADDR=1 : KSZ8081 PHY is at address 1 CONFIG_SAMA5_GMAC_AUTONEG=y : Use autonegotiation</p><p>If both EMAC and GMAC are selected, you will also need:</p><pre><code>CONFIG_SAMA5_GMAC_ISETH0=y           : GMAC is &quot;eth0&quot;; EMAC is &quot;eth1&quot;
</code></pre><p>PHY selection. Later in the configuration steps, you will need to select the KSZ9081 PHY for GMAC (See below)</p><h2 id="selecting-ethernet-over-usb-cdc-ecm-driver" tabindex="-1">Selecting Ethernet over USB (CDC ECM driver) <a class="header-anchor" href="#selecting-ethernet-over-usb-cdc-ecm-driver" aria-label="Permalink to &quot;Selecting Ethernet over USB (CDC ECM driver)&quot;">​</a></h2><p>This uses the USB 2.0 connector labeled USB-A. On the host computer you will need to configure the CDC ECM Ethernet over USB driver (see below for Linux configuration script).</p><pre><code>CONFIG_USBDEV=y
CONFIG_USBDEV_DMA=y
CONFIG_USBDEV_DUALSPEED=y
CONFIG_NET_CDCECM=y
CONFIG_NET_ETH_PKTSIZE=1514
</code></pre><p>You can also use the defconfig file in <code>boards/arm/sama5/sama5d3-xplained/configs/ethernet-over-usb-2-high-speed</code>.</p><p>Common configuration settings -----------------------------</p><p>Networking Support CONFIG_NET=y : Enable Neworking CONFIG_NET_SOCKOPTS=y : Enable socket operations CONFIG_NET_ETH_PKTSIZE=562 : Maximum packet size 1518 is more standard CONFIG_NET_TCP=y : Enable TCP/IP networking CONFIG_NET_TCPBACKLOG=y : Support TCP/IP backlog CONFIG_NET_UDP=y : Enable UDP networking CONFIG_NET_ICMP=y : Enable ICMP networking CONFIG_NET_ICMP_SOCKET=y : Needed for NSH ping command : Defaults should be okay for other options Device drivers -&gt; Network Device/PHY Support CONFIG_NETDEVICES=y : Enabled PHY selection CONFIG_ETH0_PHY_KSZ8081=y : Select the KSZ8081 PHY (for EMAC), OR CONFIG_ETH0_PHY_KSZ90x1=y : Select the KSZ9031 PHY (for GMAC)</p><p>Application Configuration -&gt; Network Utilities CONFIG_NETDB_DNSCLIENT=y : Enable host address resolution CONFIG_NETUTILS_TELNETD=y : Enable the Telnet daemon CONFIG_NETUTILS_TFTPC=y : Enable TFTP data file transfers for get and put commands CONFIG_NETUTILS_NETLIB=y : Network library support is needed CONFIG_NETUTILS_WEBCLIENT=y : Needed for wget support : Defaults should be okay for other options Application Configuration -&gt; NSH Library CONFIG_NSH_TELNET=y : Enable NSH session via Telnet CONFIG_NSH_IPADDR=0x0a000002 : Select an IP address CONFIG_NSH_DRIPADDR=0x0a000001 : IP address of gateway/host PC CONFIG_NSH_NETMASK=0xffffff00 : Netmask CONFIG_NSH_NOMAC=y : Need to make up a bogus MAC address : Defaults should be okay for other options</p><p>Using the network with NSH --------------------------</p><p>So what can you do with this networking support? First you see that NSH has several new network related commands:</p><pre><code>ifconfig, ifdown, ifup:  Commands to help manage your network
get and put:             TFTP file transfers
wget:                    HTML file transfers
ping:                    Check for access to peers on the network
Telnet console:          You can access the NSH remotely via telnet.
</code></pre><p>You can also enable other add on features like full FTP or a Web Server or XML RPC and others. There are also other features that you can enable like DHCP client (or server) or network name resolution.</p><p>By default, the IP address of the SAMA5D3-Xplained will be 10.0.0.2 and it will assume that your host is the gateway and has the IP address 10.0.0.1.</p><pre><code>nsh&gt; ifconfig
...
</code></pre><p>NOTE: In this configuration is is normal to have packet loss &gt; 0% the first time you ping due to the default handling of the ARP table.</p><p>On the host side, you should also be able to ping the SAMA5D3-Xplained:</p><pre><code>$ ping 10.0.0.2
...
</code></pre><h2 id="ethernet-over-usb-configuration-script" tabindex="-1">Ethernet Over USB Configuration Script <a class="header-anchor" href="#ethernet-over-usb-configuration-script" aria-label="Permalink to &quot;Ethernet Over USB Configuration Script&quot;">​</a></h2><p>There is a configuration script for Linux that will configure the USB Ethernet interface, it is in <code>tools/netusb.sh</code>. You can use it as follows:</p><p>Once you boot a NuttX system with the CDC ECM Ethernet over USB device, the Linux network interface will be added to your system. You should see something like the following messages in /var/log/kern.log:</p><p>[302074.552879] usb 1-2: new high-speed USB device number 107 using ehci-pci [302074.718264] usb 1-2: New USB device found, idVendor=0525, idProduct=a4a2, bcdDevice= 1.00 [302074.718267] usb 1-2: New USB device strings: Mfr=1, Product=2, SerialNumber=3 [302074.718269] usb 1-2: Product: CDC/ECM Ethernet [302074.718271] usb 1-2: Manufacturer: NuttX [302074.718272] usb 1-2: SerialNumber: 0 [302074.760638] cdc_ether 1-2:1.0 usb0: register &#39;cdc_ether&#39; at usb-0000:02:03.0-2, CDC Ethernet Device, 02:00:00:11:22:33 [302074.796215] cdc_ether 1-2:1.0 ens160u4u2: renamed from usb0</p><p>If you execute the command &#39;ifconfig -a&#39; you should see a new interface:</p><p>$ ifconfig -a</p><p>ens33: flags=4163&lt;UP,BROADCAST,RUNNING,MULTICAST&gt; mtu 1500 inet 192.168.46.156 netmask 255.255.255.0 broadcast 192.168.46.255 inet6 fe80::20c:29ff:fe57:d0f8 prefixlen 64 scopeid 0x20<code>&lt;link&gt;</code> ether 00:0c:29:57:d0:f8 txqueuelen 1000 (Ethernet) RX packets 7628014 bytes 2002078802 (2.0 GB) RX errors 0 dropped 0 overruns 0 frame 0 TX packets 6040388 bytes 5327276865 (5.3 GB) TX errors 0 dropped 0 overruns 0 carrier 0 collisions 0</p><p>ens160u4u2: flags=4163&lt;UP,BROADCAST,RUNNING,MULTICAST&gt; mtu 1500 inet6 fe80::ff:fe11:2233 prefixlen 64 scopeid 0x20<code>&lt;link&gt;</code> ether 02:00:00:11:22:33 txqueuelen 1000 (Ethernet) RX packets 36798 bytes 51705300 (51.7 MB) RX errors 0 dropped 0 overruns 0 frame 0 TX packets 24196 bytes 1312512 (1.3 MB) TX errors 0 dropped 0 overruns 0 carrier 0 collisions 0</p><p>ens33 is the host Ethernet or wireless LAN interface. ens160u4u2 is the USB Ethernet interface.</p><p>The script will bring up the interface, configure it, and set up routes and IP Tables rules so the nuttx system can access the internet:</p><p>$ sudo ./tools/netusb.sh ens33 ens160u4u2 on</p><p>This will bring down the interface, configure it, and delete routes and IP Tables rules:</p><p>$ sudo ./tools/netusb.sh ens33 ens160u4u2 off</p><p>Now that the new interface has an IP address, you can ping the NuttX box at 10.0.0.2 (or whatever IP address you configured it to have). If you configured the telnet daemon and started it, you should be able to telnet to the board using:</p><p>$ telnet 10.0.0.2</p><p>The helper script also sets up Network Address Translation (NAT) so the NuttX system can access the Internet. If that is not what you want, you can remove the iptables</p><h1 id="at25-serial-flash" tabindex="-1">AT25 Serial FLASH <a class="header-anchor" href="#at25-serial-flash" aria-label="Permalink to &quot;AT25 Serial FLASH&quot;">​</a></h1><p>Connections -----------</p><p>The SAMA5D3-Xplained board supports an options Serial DataFlash connected at MN8. The SPI connection is as follows:</p><pre><code>     MN8       SAMA5
------------- -----------------------------------------------
...
</code></pre><p>NOTE: The MN8 is not populated on my SAMAD3 Xplained board. So, as a result, these instructions would only apply if you were to have an AT25 Serial DataFlash installed in MN8.</p><p>Configuration -------------</p><pre><code>System Type -&gt; SAMA5 Peripheral Support
...
This is a test
</code></pre><h1 id="hsmci-card-slots" tabindex="-1">HSMCI Card Slots <a class="header-anchor" href="#hsmci-card-slots" aria-label="Permalink to &quot;HSMCI Card Slots&quot;">​</a></h1><p>Physical Slots --------------</p><p>The SAMA5D3-Xplained provides a two SD memory card slots: (1) a full size SD card slot (J10), and (2) a microSD memory card slot (J11).</p><p>The full size SD card slot connects via HSMCI0. The card detect discrete is available on PD17 (pulled high). The write protect discrete is tied to ground and not i savailable to software. The slot supports 8-bit wide transfer mode, but the NuttX driver currently uses only the 4-bit wide transfer mode</p><pre><code>PD17 MCI0_CD
PD1  MCI0_DA0
PD2  MCI0_DA1
PD3  MCI0_DA2
PD4  MCI0_DA3
...
PB24  MCI1_CK
PB19  MCI1_CDA
</code></pre><p>Configuration Settings ----------------------</p><p>Enabling HSMCI support. The SAMA5D3-Xplained provides a two SD memory card slots: (1) a full size SD card slot (J10), and (2) a microSD memory card slot (J11). The full size SD card slot connects via HSMCI0; the microSD connects via HSMCI1. Support for both SD slots can be enabled with the following settings:</p><pre><code>System Type-&gt;ATSAMA5 Peripheral Support
...
* Port C Host Full Speed (OHCI) only standard type A connector, J7
  lower port
</code></pre><p>The two USB host ports (only) are equipped with 500-mA high-side power switch for self-powered and bus-powered applications.</p><p>The USB device port A (J6) features a VBUS insert detection function.</p><p>Port A ------</p><pre><code>PIO  Signal Name Function
---- ----------- -------------------------------------------------------
...
Application Configuration -&gt; NSH Library
  CONFIG_NSH_ARCHINIT=y                : NSH board-initialization
</code></pre><p>file1: CONFIG_USBHOST_ISOC_DISABLE=y</p><p>NOTE: When OHCI is selected, the SAMA5 will operate at 384MHz instead of 396MHz. This is so that the PLL generates a frequency which is a multiple of the 48MHz needed for OHCI. The delay loop calibration values that are used will be off slightly because of this.</p><p>EHCI ----</p><p>Support the USB high-speed EHCI host driver can be enabled by changing the NuttX configuration file as follows. If EHCI is enabled by itself, then only high-speed devices can be supported. If OHCI is also enabled, then all low-, full-, and high speed devices will work.</p><pre><code>System Type -&gt; ATSAMA5 Peripheral Support
  CONFIG_SAMA5_UHPHS=y                 : USB Host High Speed
...
Library Routines
  CONFIG_SCHED_WORKQUEUE=y          : Driver needs work queue support
</code></pre><h1 id="shields" tabindex="-1">Shields <a class="header-anchor" href="#shields" aria-label="Permalink to &quot;Shields&quot;">​</a></h1><p>Support is built in for the following shields:</p><h2 id="itead-joystick-shield" tabindex="-1">Itead Joystick Shield <a class="header-anchor" href="#itead-joystick-shield" aria-label="Permalink to &quot;Itead Joystick Shield&quot;">​</a></h2><p>See <a href="http://imall.iteadstudio.com/im120417014.html" target="_blank" rel="noreferrer">http://imall.iteadstudio.com/im120417014.html</a> for more information about this joystick.</p><p>Itead Joystick Connection:</p><pre><code>--------- ----------------- ---------------------------------
...
</code></pre><p>Possible conflicts:</p><pre><code>---- ----- --------------------------------------------------
...
--------- ----------------------- ---------------------------
</code></pre><p>Itead Joystick configuration settings:</p><pre><code>System Type -&gt; SAMA5 Peripheral Support
  CONFIG_SAMA5_ADC=y               : Enable ADC driver support
  CONFIG_SAMA5_TC0=y               : Enable the Timer/counter library need for periodic sampling
...
</code></pre><p>STATUS: 2014-12-03: As nearly I can tell, the Itead Joystick shield cannot be used with the SAMA5D3-Xplained. I believe that the EMAC PHY chip is enableed and since it shares pins with the Joystick, it interferes with the Joystick inputs. There is probably more wrong than this; perhaps I am not setting up the pins correctly. But having seen the states of the button output pins change when powering up the board, I have lost hope of getting the shield to work on this board. I leave the implementation in place only for reference.</p><h1 id="sama5d3-xplained-configuration-options" tabindex="-1">SAMA5D3-Xplained Configuration Options <a class="header-anchor" href="#sama5d3-xplained-configuration-options" aria-label="Permalink to &quot;SAMA5D3-Xplained Configuration Options&quot;">​</a></h1><p>CONFIG_ARCH - Identifies the arch/ subdirectory. This should be set to:</p><pre><code>CONFIG_ARCH=&quot;arm&quot;
...
</code></pre><p>There may be issues with some of these configurations. See the details before of the status of individual configurations.</p><p>Now for the gory details:</p><p>bridge:</p><pre><code>This is a simple testing that exercises EMAC and GEMAC for a simple
