import{_ as n,c as t,al as o,o as a}from"./chunks/framework.NFAqBSgQ.js";const p=JSON.parse('{"title":"README.txt","description":"","frontmatter":{},"headers":[],"relativePath":"en/platforms/arm/lpc17xx/boards/open1788/README.md","filePath":"en/platforms/arm/lpc17xx/boards/open1788/README.md"}'),i={name:"en/platforms/arm/lpc17xx/boards/open1788/README.md"};function s(r,e,l,d,h,u){return a(),t("div",null,e[0]||(e[0]=[o(`<h1 id="readme-txt" tabindex="-1">README.txt <a class="header-anchor" href="#readme-txt" aria-label="Permalink to &quot;README.txt&quot;">​</a></h1><p>This README file discusses the port of NuttX to the WaveShare Open1788 board: See <a href="http://wvshare.com/product/Open1788-Standard.htm" target="_blank" rel="noreferrer">http://wvshare.com/product/Open1788-Standard.htm</a>. This board features the NXP LPC1788 MCU</p><h1 id="contents" tabindex="-1">CONTENTS <a class="header-anchor" href="#contents" aria-label="Permalink to &quot;CONTENTS&quot;">​</a></h1><p>o LEDs o Buttons o Serial Console o Using OpenOCD with the Olimex ARM-USB-OCD o Loading Code with the ISP Board o Configuration</p><h1 id="leds" tabindex="-1">LEDs <a class="header-anchor" href="#leds" aria-label="Permalink to &quot;LEDs&quot;">​</a></h1><p>The Open1788 base board has four user LEDs</p><pre><code>LED1 : Connected to P1[14]
LED2 : Connected to P0[16]
LED3 : Connected to P1[13]
LED4 : Connected to P4[27]
</code></pre><p>If CONFIG_ARCH_LEDS is not defined, then the user can control the LEDs in any way using the definitions provided in the board.h header file.</p><p>If CONFIG_ARCH_LEDs is defined, then NuttX will control the 3 LEDs on the WaveShare Open1788K. The following definitions describe how NuttX controls the LEDs: LED1 LED2 LED3 LED4 LED_STARTED OFF OFF OFF OFF LED_HEAPALLOCATE ON OFF OFF OFF LED_IRQSENABLED OFF ON OFF OFF LED_STACKCREATED ON ON OFF OFF LED_INIRQ LED3 glows, on while in interrupt LED_SIGNAL LED3 glows, on while in signal handler LED_ASSERTION LED3 glows, on while in assertion LED_PANIC LED3 Flashes at 2Hz LED_IDLE LED glows: ON while active; OFF while sleeping</p><h1 id="buttons" tabindex="-1">Buttons <a class="header-anchor" href="#buttons" aria-label="Permalink to &quot;Buttons&quot;">​</a></h1><p>The Open1788K supports several buttons:</p><pre><code>USER1           : Connected to P4[26]
USER2           : Connected to P2[22]
USER3           : Connected to P0[10]
</code></pre><p>And a Joystick</p><pre><code>JOY_A           : Connected to P2[25]
JOY_B           : Connected to P2[26]
JOY_C           : Connected to P2[23]
JOY_D           : Connected to P2[19]
JOY_CTR         : Connected to P0[14]
</code></pre><p>These can be accessed using the definitions and interfaces defined in the board.h header file.</p><h1 id="serial-console" tabindex="-1">Serial Console <a class="header-anchor" href="#serial-console" aria-label="Permalink to &quot;Serial Console&quot;">​</a></h1><p>By Default, UART0 is used as the serial console in all configurations. This may be connected to your computer via an external RS-232 driver or via the WaveShare USB ISP/VCOM module.</p><p>As an option, UART1 can also be used for the serial console. You might want, to do this, for example, if you use UART0 for the ISP function and you want to use a different UART for console output. UART1 can be configured as the serial console by changing the configuration as follows:</p><pre><code>System Type:
...
</code></pre><p>You should also remove the LED2 jumper so that the RXD input does not attempt to drive LED2 as well (However, this does not seem to interfere with data receipt).</p><p>NOTE: If you intend to use LEDs with UART1, then you might want to redesign some of the LED logic in the src/ subdirectory so that it does not attempt to use LED2.</p><h1 id="using-openocd-with-the-olimex-arm-usb-ocd" tabindex="-1">Using OpenOCD with the Olimex ARM-USB-OCD <a class="header-anchor" href="#using-openocd-with-the-olimex-arm-usb-ocd" aria-label="Permalink to &quot;Using OpenOCD with the Olimex ARM-USB-OCD&quot;">​</a></h1><p>Building OpenOCD under Cygwin:</p><pre><code>Refer to boards/olimex-lpc1766stk/README.txt
...
</code></pre><p>knxterm -------</p><pre><code>This is another protected mode build.  This configuration was used to
...
</code></pre><h2 id="nxlines" tabindex="-1">nxlines <a class="header-anchor" href="#nxlines" aria-label="Permalink to &quot;nxlines&quot;">​</a></h2><p>Configures the graphics example located at examples/nsh. This configuration enables SDRAM to hold the LCD framebuffer and enables the LPC178x LCD driver in order to support the WaveShare 4.3 inch TFT panel.</p><pre><code>NOTES:
...
2. In this configuration, the SDRAM is not added to heap but is
   dedicated to supporting an LCD frame buffer at address 0xa0010000.
</code></pre><p>pdcurses -------</p><pre><code>A simple NSH configuration used for some basic (non-graphic) debug of
...
</code></pre><p>pwfb: -----</p><pre><code>This configuration uses the test at apps/examples/pwfb to verify the
...
</code></pre>`,57)]))}const f=n(i,[["render",s]]);export{p as __pageData,f as default};
