import{_ as n,c as t,al as o,o as i}from"./chunks/framework.NFAqBSgQ.js";const h=JSON.parse('{"title":"","description":"","frontmatter":{},"headers":[],"relativePath":"en/platforms/arm/tiva/boards/lm3s6965-ek/README.md","filePath":"en/platforms/arm/tiva/boards/lm3s6965-ek/README.md"}'),r={name:"en/platforms/arm/tiva/boards/lm3s6965-ek/README.md"};function a(s,e,l,c,d,u){return i(),t("div",null,e[0]||(e[0]=[o(`<p>README ^^^^^^</p><p>README for NuttX port to the Stellaris LMS36965 Evaluation Kit</p><p>Contents ^^^^^^^^</p><p>Stellaris LMS36965 Evaluation Kit OLED Using OpenOCD and GDB with an FT2232 JTAG emulator USB Device Controller Functions Stellaris LM3S6965 Evaluation Kit Configuration Options Configurations</p><p>Stellaris LMS36965 Evaluation Kit ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^</p><p>The Stellaris LM3S6965 Evaluation Board includes the following features:</p><p>o Stellaris LM3S6965 microcontroller with fully-integrated 10/100 embedded Ethernet controller o Simple setup; USB cable provides serial communication, debugging, and power o OLED graphics display with 128 x 96 pixel resolution o User LED, navigation switches, and select pushbuttons o Magnetic speaker o LM3S6965 I/O available on labeled break-out pads o Standard ARM(R) 20-pin JTAG debug connector with input and output modes o USB interface for debugging and power supply o MicroSD card slot</p><p>Features of the LM3S6965 Microcontroller</p><p>o 32-bit RISC performance using ARM(R) Cortex-M3 v7M architecture - 50-MHz operation - Hardware-division and single-cycle-multiplication - Integrated Nested Vectored Interrupt Controller (NVIC) - 42 interrupt channels with eight priority levels o 256 KB single-cycle flash o 64 KB single-cycle SRAM o Four general-purpose 32-bit timers o Integrated Ethernet MAC and PHY o Three fully programmable 16C550-type UARTs o Four 10-bit channels (inputs) when used as single-ended inputs o Two independent integrated analog comparators o Two I2C modules o Three PWM generator blocks - One 16-bit counter - Two comparators - Produces two independent PWM signals - One dead-band generator o Two QEI modules with position integrator for tracking encoder position o 0 to 42 GPIOs, depending on user configuration o On-chip low drop-out (LDO) voltage regulator</p><p>GPIO Usage</p><p>PIN SIGNAL EVB Function</p><hr><p>26 PA0/U0RX Virtual COM port receive 27 PA1/U0TX Virtual COM port transmit 10 PD0/IDX0 SD card chip select 11 PD1/PWM1 Sound 30 PA4/SSI0RX SD card data out 31 PA5/SSI0TX SD card and OLED display data in 28 PA2/SSI0CLK SD card and OLED display clock 22 PC7/PHB0 OLED display data/control select 29 PA3/SSI0FSS OLED display chip select 73 PE1/PWM5 Down switch 74 PE2/PHB1 Left switch 72 PE0/PWM4 Up switch 75 PE3/PHA1 Right switch 61 PF1/IDX1 Select switch 47 PF0/PWM0 User LED 23 PC6/CCP3 Enable +15 V</p><p>OLED ^^^^</p><p>The Evaluation Kit includes an OLED graphics display. Features:</p><ul><li>RiT P14201 series display</li><li>128 columns by 96 rows</li><li>4-bit, 16-level gray scale.</li><li>High-contrast (typ. 500:1)</li><li>Excellent brightness (120 cd/m2)</li><li>Fast 10 us response.</li></ul><p>The OLED display has a built-in controller IC with synchronous serial and parallel interfaces (SSD1329). Synchronous serial (SSI) is used on the EVB. The SSI port is shared with the microSD card slot.</p><ul><li>PC7: OLED display data/control select (D/Cn)</li><li>PA3: OLED display chip select (CSn)</li></ul><p>NOTE: Newer versions of the LM3S6965 Evaluation Kit has an OSAM 128x64x4 OLED display. Some tweaks to drivers/lcd/p14201.c would be required to support that LCD.</p><p>Using OpenOCD and GDB with an FT2232 JTAG emulator ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^</p><p>Building OpenOCD under Cygwin:</p><pre><code>Refer to boards/olimex-lpc1766stk/README.txt
</code></pre><p>Installing OpenOCD in Linux:</p><pre><code>sudo apt-get install openocd
</code></pre><p>Helper Scripts.</p><pre><code>I have been using the on-board FT2232 JTAG/SWD/SWO interface.  OpenOCD
...
</code></pre><p>USB Device Controller Functions ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^</p><p>Device Overview</p><pre><code>An FT2232 device from Future Technology Devices International Ltd manages
...
</code></pre><p>Stellaris LM3S6965 Evaluation Kit Configuration Options ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^</p><pre><code>CONFIG_ARCH - Identifies the arch/ subdirectory.  This should
   be set to:
...
</code></pre><p>There are configurations for disabling support for interrupts GPIO ports. GPIOJ must be disabled because it does not exist on the LM3S6965. Additional interrupt support can be disabled if desired to reduce memory footprint.</p><pre><code>CONFIG_TIVA_GPIOA_IRQS=y
...
</code></pre><p>LM3S6965 specific device driver settings</p><pre><code>CONFIG_UARTn_SERIAL_CONSOLE - selects the UARTn for the
...
</code></pre><p>Configurations ^^^^^^^^^^^^^^</p><p>Each Stellaris LM3S6965 Evaluation Kit configuration is maintained in a sub-directory and can be selected as follow:</p><pre><code>tools/configure.sh lm3s6965-ek:&lt;subdir&gt;
</code></pre><p>Where <code>&lt;subdir&gt;</code> is one of the following:</p><p>discover: A configuration for the UDP discovery tool at apps/examples/discover. Contributed by Max Holtzberg.</p><pre><code>NOTES:
...
4. Network File System (NFS) support can be added by setting the
  ...
  CONFIG_NFS=y
</code></pre><p>nx: And example using the NuttX graphics system (NX). This example uses the P14201 OLED driver.</p><pre><code>NOTES:
1. This configuration uses the mconf-based configuration tool.  To
   change this configuration using that tool, you should:
...
2. Default platform/toolchain:
   ...
   CONFIG_ARM_TOOLCHAIN_BUILDROOT=y : Buildroot (arm-nuttx-elf-gcc)
   CONFIG_RAW_BINARY=y                 : Output formats: ELF and raw binary
</code></pre><p>qemu-flat: An example config with FLAT memory model to run on qemu.</p><pre><code>    ./tools/configure.sh lm3s6965-ek:qemu-flat
    make
...
</code></pre><p>qemu-kostest: An example config with PROTECTED memory model to run on qemu.</p><pre><code>    ./tools/configure.sh lm3s6965-ek:qemu-kostest
    make
    qemu-system-arm -semihosting \\
    -M lm3s6965evb \\
    -device loader,file=nuttx.bin,addr=0x00000000 \\
    ...
    -serial mon:stdio -nographic
</code></pre><p>qemu-protected: An example config with PROTECTED memory model to run on qemu.</p><pre><code>    ./tools/configure.sh lm3s6965-ek:qemu-protected
    make
    qemu-system-arm -semihosting \\
    -M lm3s6965evb \\
    -device loader,file=nuttx.bin,addr=0x00000000 \\
    ...
    -nic user,id=user0 \\
    -serial mon:stdio -nographic
</code></pre><p>tcpecho: This configuration builds the simple TCP echo example based on W.Richard Steven UNIX Programming book to ensure correct usage of the socket API. Contributed by Max Holtzberg.</p><pre><code>NOTES:
1. This configuration uses the mconf-based configuration tool.  To
   change this configuration using that tool, you should:
...
3. As it is configured now, you MUST have a network connected.
   ...
   driver is waiting for the network to come up.  That is probably
   a bug in the Ethernet driver behavior!
</code></pre>`,61)]))}const m=n(r,[["render",a]]);export{h as __pageData,m as default};
