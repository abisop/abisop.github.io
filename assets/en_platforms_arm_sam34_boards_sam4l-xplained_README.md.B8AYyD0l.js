import{_ as n,c as t,al as o,o as i}from"./chunks/framework.NFAqBSgQ.js";const _=JSON.parse('{"title":"","description":"","frontmatter":{},"headers":[],"relativePath":"en/platforms/arm/sam34/boards/sam4l-xplained/README.md","filePath":"en/platforms/arm/sam34/boards/sam4l-xplained/README.md"}'),a={name:"en/platforms/arm/sam34/boards/sam4l-xplained/README.md"};function s(r,e,l,C,I,S){return i(),t("div",null,e[0]||(e[0]=[o(`<p>README ^^^^^^</p><p>This README discusses issues unique to NuttX configurations for the Atmel SAM4L Xplained Pro development board. This board features the ATSAM4LC4C MCU.</p><p>The SAM4L Xplained Pro Starter Kit is bundled with four modules:</p><ol><li>I/O1 - An MMC/SD card slot, PWM LED control, ADC light sensor, UART loopback, TWI AT30TSE758 Temperature sensor.</li><li>OLED1 - An OLED plus 3 additional switches and 3 additional LEDs</li><li>SLCD1 - A segment LCD that connects directly to the &quot;EXT5 SEGMENT LCD&quot; connector</li><li>PROTO1 - A prototyping board with logic on board (other than power-related logic).</li></ol><p>Contents ^^^^^^^^</p><ul><li>Modules</li><li>LEDs</li><li>Serial Consoles</li><li>SAM4L Xplained Pro-specific Configuration Options</li><li>Configurations</li></ul><p>Modules ^^^^^^^ The SAM4L Xplained Pro Starter Kit is bundled with four modules:</p><h2 id="i-o1" tabindex="-1">I/O1 <a class="header-anchor" href="#i-o1" aria-label="Permalink to &quot;I/O1&quot;">​</a></h2><p>The primary function of this module is to provide SD card support, but the full list of modules features include:</p><pre><code>- microSD card connector (SPI interface)
- PWM (LED control)
- ADC (light sensor)
- UART loopback
...
See the set-up in the discussion of the nsh configuration below for other
required configuration options.
</code></pre><h2 id="slcd1" tabindex="-1">SLCD1 <a class="header-anchor" href="#slcd1" aria-label="Permalink to &quot;SLCD1&quot;">​</a></h2><p>This module provides a A segment LCD that connects directly to the &quot;EXT5 SEGMENT LCD&quot; connector</p><pre><code>Configuration Options:
----------------------
...
See the set-up in the discussion of the nsh configuration below for other
required configuration options.
</code></pre><h2 id="proto1" tabindex="-1">PROTO1 <a class="header-anchor" href="#proto1" aria-label="Permalink to &quot;PROTO1&quot;">​</a></h2><p>A prototyping board with logic on board (other than power-related logic). There is no built-in support for the PROTO1 module.</p><p>LEDs ^^^^ There are three LEDs on board the SAM4L Xplained Pro board: The EDBG controls two of the LEDs, a power LED and a status LED. There is only one user controllable LED, a yellow LED labeled LED0 near the SAM4L USB connector.</p><p>This LED is controlled by PC07 and LED0 can be activated by driving the PC07 to GND.</p><p>When CONFIG_ARCH_LEDS is defined in the NuttX configuration, NuttX will control LED0 as follows:</p><pre><code>SYMBOL              Meaning                 LED0
------------------- ----------------------- ------
...
</code></pre><p>Thus is LED0 is statically on, NuttX has successfully booted and is, apparently, running normmally. If LED0 is flashing at approximately 2Hz, then a fatal error has been detected and the system has halted.</p><p>Serial Consoles ^^^^^^^^^^^^^^^</p><p>USART0 ------</p><p>USART0 is available on connectors EXT1 and EXT4</p><pre><code>EXT1  EXT4  GPIO  Function
----  ---- ------ -----------
 13    13   PB00  USART0_RXD
 14    14   PB01  USART0_TXD
 19    19         GND
 20    20         VCC
</code></pre><p>If you have a TTL to RS-232 converter then this is the most convenient serial console to use. It is the default in all of these configurations. An option is to use the virtual COM port.</p><p>Virtual COM Port ----------------</p><p>The SAM4L Xplained Pro contains an Embedded Debugger (EDBG) that can be used to program and debug the ATSAM4LC4C using Serial Wire Debug (SWD). The Embedded debugger also include a Virtual Com port interface over USART1. Virtual COM port connections:</p><pre><code>PC26 USART1 RXD
PC27 USART1 TXD
</code></pre><p>SAM4L Xplained Pro-specific Configuration Options ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^</p><pre><code>CONFIG_ARCH - Identifies the arch/ subdirectory.  This should
   be set to:
...
</code></pre><p>Individual subsystems can be enabled:</p><pre><code>CPU
---
CONFIG_SAM34_OCD
...
</code></pre><p>ST91SAM4L specific device driver settings</p><pre><code>CONFIG_U[S]ARTn_SERIAL_CONSOLE - selects the USARTn (n=0,1,2,3) or UART
...
</code></pre><p>Configurations ^^^^^^^^^^^^^^</p><p>Each SAM4L Xplained Pro configuration is maintained in a sub-directory and can be selected as follow:</p><pre><code>tools/configure.sh sam4l-xplained:&lt;subdir&gt;
...
</code></pre>`,56)]))}const h=n(a,[["render",s]]);export{_ as __pageData,h as default};
