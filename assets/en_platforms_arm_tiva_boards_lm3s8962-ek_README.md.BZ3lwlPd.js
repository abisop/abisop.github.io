...
</code></pre><p>Stellaris LM3S8962 Evaluation Kit Configuration Options ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^</p><pre><code>CONFIG_ARCH - Identifies the arch/ subdirectory.  This should
   be set to:
...
</code></pre><p>There are configurations for disabling support for interrupts GPIO ports. GPIOJ must be disabled because it does not exist on the LM3S8962. Additional interrupt support can be disabled if desired to reduce memory footprint.</p><pre><code>CONFIG_TIVA_GPIOA_IRQS=y
...
</code></pre><p>LM3S8962 specific device driver settings</p><pre><code>CONFIG_UARTn_SERIAL_CONSOLE - selects the UARTn for the
...
</code></pre><p>Configurations ^^^^^^^^^^^^^^</p><p>Each Stellaris LM3S8962 Evaluation Kit configuration is maintained in a sub-directory and can be selected as follow:</p><pre><code>tools/configure.sh lm3s8962-ek:&lt;subdir&gt;
</code></pre><p>Where <code>&lt;subdir&gt;</code> is one of the following:</p><p>nsh: Configures the NuttShell (nsh) located at examples/nsh.</p><pre><code>NOTES:
...
4. As it is configured now, you MUST have a network connected. Otherwise,
   ...
   driver behavior!
</code></pre><p>nx An example using the NuttX graphics system (NX). This example focuses on general window controls, movement, mouse and keyboard input.</p><pre><code>NOTES:
...
2. This configuration uses Cygwin under Windows with the NuttX buildroot
   tool chain.  This is eaily change by modifying the following:
     ...
     CONFIG_WINDOWS_CYGWIN=y
     CONFIG_ARM_TOOLCHAIN_BUILDROOT=y
</code></pre>`,40)]))}const I=t(r,[["render",a]]);export{h as __pageData,I as default};
