import{_ as t,c as n,al as o,o as i}from"./chunks/framework.NFAqBSgQ.js";const p=JSON.parse('{"title":"README","description":"","frontmatter":{},"headers":[],"relativePath":"en/platforms/arm/lpc31xx/boards/olimex-lpc-h3131/README.md","filePath":"en/platforms/arm/lpc31xx/boards/olimex-lpc-h3131/README.md"}'),a={name:"en/platforms/arm/lpc31xx/boards/olimex-lpc-h3131/README.md"};function r(s,e,l,d,h,u){return i(),n("div",null,e[0]||(e[0]=[o(`<h1 id="readme" tabindex="-1">README <a class="header-anchor" href="#readme" aria-label="Permalink to &quot;README&quot;">​</a></h1><p>This README file discusses the port of NuttX to the Olimex LPC-H3131 board.</p><p>NOTE: This is a minimal port to the Olimex LPC-H3131. According to Olimex documentation, the LPC-H3131 is similar in design to the Embedded Artists EA3131. As a consequence, it should be possible to leverage additional functionality from boards/arm/lpc31xx/ea3131 without too much difficulty.</p><h1 id="contents" tabindex="-1">Contents <a class="header-anchor" href="#contents" aria-label="Permalink to &quot;Contents&quot;">​</a></h1><p>o Development Environment o GNU Toolchain Options o IDEs o NuttX buildroot Toolchain o Boot Sequence o Buttons and LEDs o Image Format o Image Download to ISRAM o Using OpenOCD and GDB o ARM/LPC-H3131-specific Configuration Options o Configurations</p><h1 id="development-environment" tabindex="-1">Development Environment <a class="header-anchor" href="#development-environment" aria-label="Permalink to &quot;Development Environment&quot;">​</a></h1><p>Either Linux or Cygwin on Windows can be used for the development environment. The source has been built only using the GNU toolchain (see below). Other toolchains will likely cause problems.</p><h1 id="gnu-toolchain-options" tabindex="-1">GNU Toolchain Options <a class="header-anchor" href="#gnu-toolchain-options" aria-label="Permalink to &quot;GNU Toolchain Options&quot;">​</a></h1><p>The NuttX make system has been modified to support the following different toolchain options.</p><ol><li>The NuttX buildroot Toolchain (see below), or</li><li>Any generic arm-none-eabi GNU toolchain.</li></ol><p>All testing has been conducted using the NuttX buildroot toolchain. To use a different toolchain, you simply need to modify the configuration. As an example:</p><pre><code>CONFIG_ARM_TOOLCHAIN_GNU_EABI : Generic arm-none-eabi toolchain
...
</code></pre><p>Thus if LED2 is statically on, NuttX has successfully booted and is, apparently, running normmally. If LED1 is flashing at approximately 2Hz, then a fatal error has been detected and the system has halted.</p><p>NOTE: That LED2 is not used after completion of booting and may be used by other board-specific logic.</p><h1 id="image-format" tabindex="-1">Image Format <a class="header-anchor" href="#image-format" aria-label="Permalink to &quot;Image Format&quot;">​</a></h1><p>In order to use the bootrom bootloader, a special header must be added to the beginning of the binary image that includes information about the binary (things like the entry point, the size, and CRC&#39;s to verify the image.</p><p>NXP provides a Windows program to append such a header to the binary image. However, (1) that program won&#39;t run under Linux, and (2) when I try it under WinXP, Symantec immediately claims that the program is misbehaving and deletes it!</p><p>To work around both of these issues, I have created a small program under boards/olimex-lpc-h3131/tools to add the header. This program can be built under either Linux or Cygwin (and probably other tool environments as well). That tool can be built as follows:</p><ul><li>cd boards/olimex-lpc-h3131/tools</li><li>make</li></ul><p>Then, to build the NuttX binary ready to load with the bootloader, just following these steps:</p><ul><li>tools/configure.sh olimex-lpc-h3131:ostest # (using the ostest configuration for this example)</li><li>cd .. # Set up environment</li><li>make # Make NuttX. This will produce nuttx.bin</li><li>mklpc.sh # Make the bootloader binary (nuttx.lpc)</li></ul><p>NOTES:</p><pre><code>1. Make sure to set your PATH variable appropriately or use the full path
...
</code></pre><h1 id="image-download-to-isram" tabindex="-1">Image Download to ISRAM <a class="header-anchor" href="#image-download-to-isram" aria-label="Permalink to &quot;Image Download to ISRAM&quot;">​</a></h1><p>Assuming that you already have the FTDI driver installed*, then here is the are the steps that I use for loading new code into the LPC-H3131:</p><ol><li><p>Create the bootloader binary, nuttx.lpc, as described above.</p></li><li><p>With the power off, set the boot jumpers to enable booting from UART. The boot jumpers are the block of three jumper just in-board from the JTAG connector; Jumper pair 1-2 is the pair furthest from the JTAG connector:</p><p>1-2: Closed 3-4: Closed 5-6: Open</p></li><li><p>Connected the LPC-H3131 using the FTDI USB port (not the lpc3131 USB port) This will power up the LPC-H3131 and start the bootloader.</p></li><li><p>Start a terminal emulator (such as TeraTerm) at 115200 8NI.</p></li><li><p>Reset the LPC-H3131 and you should see:</p><p>LPC31xx READY FOR PLAIN IMAGE&gt;</p></li><li><p>Send the nuttx.lpc file and you should see:</p><p>Download finished</p></li></ol><p>That will load the NuttX binary into ISRAM and attempt to execute it.</p><p>*See the LPC313x documentation if you do not have the FTDI driver installed.</p><p>TeraTerm Note: This is how to send a file from TeraTerm. It is essentially step 6 exploded in more detail for the case of TeraTerm:</p><ol><li><p>Start the ROM bootloader as described above.</p></li><li><p>At the &quot;LPC31xx READY FOR PLAIN IMAGE&gt;&quot; prompt, open the File menu and select the &quot;Send File...&quot; option.</p></li><li><p>Select the file to send,</p></li><li><p>Before &quot;Open&quot; -ing the file MAKE SURE TO CHECK THE &quot;Binary&quot; BOX! This has cost me a few hours a few times because I forget to do this. The program will NOT RUN is sent non-binary.</p><p>[NO, I am not SHOUTING. I am just making sure that I never forget to do this again].</p></li><li><p>&quot;Open&quot;-ing the file will send it to the ROM bootloader.</p></li><li><p>You should see &quot;Download finished&quot; from the bootloader followed immediately by any serial console output from your program.</p></li></ol><h1 id="using-openocd-and-gdb" tabindex="-1">Using OpenOCD and GDB <a class="header-anchor" href="#using-openocd-and-gdb" aria-label="Permalink to &quot;Using OpenOCD and GDB&quot;">​</a></h1><p>[NOTE: As of this writing, my OpenOCD script does NOT work. It fails because it is unable to halt the LPC3131. So, unfortunately, OpenOCD is not a option right now.]</p><p>I have been using the Olimex ARM-USB-OCD JTAG debugger with the LPC-H3131 (<a href="http://www.olimex.com" target="_blank" rel="noreferrer">http://www.olimex.com</a>). The OpenOCD configuration file is here: tools/armusbocb.cfg. There is also a script on the tools directory that I used to start the OpenOCD daemon on my system called oocd.sh. That script would probably require some modifications to work in another environment:</p><pre><code>- possibly the value of OPENOCD_PATH
- If you are working under Linux you will need to change any
  occurrences of \`cygpath -w blablabla\` to just blablabla
</code></pre><p>Then you should be able to start the OpenOCD daemon like:</p><pre><code>boards/olimex-lpc-h3131/tools/oocd.sh $PWD
</code></pre><p>Where it is assumed that you are executing oocd.sh from the top level directory where NuttX is installed.</p><p>Once the OpenOCD daemon has been started, you can connect to it via GDB using the following GDB command:</p><p>arm-nuttx-elf-gdb (gdb) target remote localhost:3333</p><p>And you can load the NuttX ELF file:</p><p>(gdb) symbol-file nuttx (gdb) load nuttx</p><h1 id="arm-lpc-h3131-specific-configuration-options" tabindex="-1">ARM/LPC-H3131-specific Configuration Options <a class="header-anchor" href="#arm-lpc-h3131-specific-configuration-options" aria-label="Permalink to &quot;ARM/LPC-H3131-specific Configuration Options&quot;">​</a></h1><pre><code>CONFIG_ARCH - Identifies the arch/ subdirectory.  This should
   be set to:
...
</code></pre><h1 id="configurations" tabindex="-1">Configurations <a class="header-anchor" href="#configurations" aria-label="Permalink to &quot;Configurations&quot;">​</a></h1><h1 id="configurations-1" tabindex="-1">Configurations <a class="header-anchor" href="#configurations-1" aria-label="Permalink to &quot;Configurations&quot;">​</a></h1><h2 id="information-common-to-all-configurations" tabindex="-1">Information Common to All Configurations <a class="header-anchor" href="#information-common-to-all-configurations" aria-label="Permalink to &quot;Information Common to All Configurations&quot;">​</a></h2><p>Each LPC-H3131 configuration is maintained in a sub-directory and can be selected as follow:</p><pre><code>tools/configure.sh olimex-lpc-h3131:&lt;subdir&gt;
...
</code></pre><ol start="2"><li><p>Unless stated otherwise, all configurations generate console output on the UART0 associated with the FT232RL USB-to UART converter.</p></li><li><p>Unless otherwise stated, the configurations are setup for Windows undery Cygwin. This can, however, be easily reconfigured.</p></li><li><p>All of these configurations use the Code Sourcery for Windows toolchain (unless stated otherwise in the description of the configuration). That toolchain selection can easily be reconfigured using &#39;make menuconfig&#39;. Here are the relevant current settings:</p><p>Build Setup: CONFIG_HOST_WINDOWS=y : Microsoft Windows CONFIG_WINDOWS_CYGWIN=y : Using Cygwin or other POSIX environment</p><p>System Type -&gt; Toolchain: CONFIG_ARM_TOOLCHAIN_GNU_EABI=y : GNU EABI toolchain for windows</p></li></ol><p>Configuration sub-directories -----------------------------</p><p>nsh: Configures the NuttShell (nsh) located at examples/nsh. The Configuration enables only the serial NSH interface.</p><pre><code>General Configuration.  These are easily change by modifying the NuttX
configuration:
...
</code></pre>`,95)]))}const m=t(a,[["render",r]]);export{p as __pageData,m as default};
