...
</code></pre><ul><li>If LED1, LED2, LED3 are statically on, then NuttX probably failed to boot and these LEDs will give you some indication of where the failure was ** The normal state is LED3 ON and LED1 faintly glowing. This faint glow is because of timer interrupts that result in the LED being illuminated on a small proportion of the time. *** LED2 may also flicker normally if signals are processed.</li></ul><h1 id="twr-k60n512-specific-configuration-options" tabindex="-1">TWR-K60N512-specific Configuration Options <a class="header-anchor" href="#twr-k60n512-specific-configuration-options" aria-label="Permalink to &quot;TWR-K60N512-specific Configuration Options&quot;">​</a></h1><pre><code>CONFIG_ARCH - Identifies the arch/ subdirectory.  This should
   be set to:
...
</code></pre><p>Kinetis K60 specific device driver settings</p><pre><code>CONFIG_UARTn_SERIAL_CONSOLE - selects the UARTn (n=0..5) for the
...
</code></pre><h1 id="configurations" tabindex="-1">Configurations <a class="header-anchor" href="#configurations" aria-label="Permalink to &quot;Configurations&quot;">​</a></h1><p>Each TWR-K60N512 configuration is maintained in a sub-directory and can be selected as follow:</p><pre><code>tools/configure.sh twr-k60n512:&lt;subdir&gt;
</code></pre><p>Where <code>&lt;subdir&gt;</code> is one of the following:</p><h2 id="nsh" tabindex="-1">nsh: <a class="header-anchor" href="#nsh" aria-label="Permalink to &quot;nsh:&quot;">​</a></h2><p>Configures the NuttShell (nsh) located at apps/examples/nsh. The Configuration enables both the serial and telnet NSH interfaces. Support for the board&#39;s SPI-based MicroSD card is included (but not passing tests as of this writing).</p><pre><code>NOTES:
...
</code></pre>`,57)]))}const E=n(i,[["render",T]]);export{S as __pageData,E as default};
